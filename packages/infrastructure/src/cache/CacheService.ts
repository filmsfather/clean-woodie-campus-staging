...
export const CacheKeys = {
  ...
  SRS_PROBLEM_PERFORMANCE: (problemId: string) => `srs:performance:problem:${problemId}`,
  ...
  // Problems
  PROBLEM_DETAIL: (problemId: string) => `problem:detail:${problemId}`,
  PROBLEM_STATS: (problemId: string) => `problem:stats:${problemId}`,
  TEACHER_PROBLEMS: (teacherId: string, filters?: string) => 
    `problems:teacher:${teacherId}${filters ? `:${filters}` : ''}`,
  PROBLEM_SEARCH: (query: string, filters?: string) => 
    `problems:search:${query}${filters ? `:${filters}` : ''}`,
  PROBLEMS_BY_TAGS: (tags: string[], filters?: string) => 
    `problems:tags:${tags.join(',')}${filters ? `:${filters}` : ''}`,
  POPULAR_PROBLEMS: (limit: number) => `problems:popular:${limit}`,
  
  // Aggregated Data
  DAILY_STATS: (date: string) => `aggregates:daily:${date}`,
  WEEKLY_STATS: (weekStart: string) => `aggregates:weekly:${weekStart}`,
  PROBLEM_SET_AGGREGATES: (problemSetId: string) => `aggregates:problem_set:${problemSetId}`,
  SYSTEM_STATS: (date: string) => `aggregates:system:${date}`,
  
  // Batch Jobs
  BATCH_JOB_STATUS: (jobId: string) => `batch:status:${jobId}`,
  ...
  NOTIFICATION_SETTINGS: (userId: string) => `notifications:settings:${userId}`,
  
  // Generic patterns for invalidation
  DASHBOARD_PATTERN: (userId: string) => `dashboard:*:${userId}`,
  SRS_PATTERN: (studentId: string) => `srs:*:${studentId}`,
  PROGRESS_PATTERN: (studentId: string) => `progress:*:${studentId}`,
  PROBLEM_PATTERN: (problemId: string) => `problem:*:${problemId}`,
  TEACHER_PATTERN: (teacherId: string) => `*:teacher:${teacherId}*`,
} as const
