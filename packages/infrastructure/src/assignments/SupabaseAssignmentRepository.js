...
export class SupabaseAssignmentRepository {
    ...
    async save(assignment) {
        try {
            ...
            if (assignmentError) {
                return Result.fail(`Failed to save assignment: ${assignmentError.message}`);
            }
            ...
            if (deleteTargetsError) {
                return Result.fail(`Failed to delete existing targets: ${deleteTargetsError.message}`);
            }
            // 새 배정 대상들 저장
            if (persistence.targets && persistence.targets.length > 0) {
                ...
                if (targetsError) {
                    return Result.fail(`Failed to save assignment targets: ${targetsError.message}`);
                }
            }
            return Result.ok();
        }
        catch (error) {
            return Result.fail(`Unexpected error saving assignment: ${error}`);
        }
    }
    async findById(id) {
        try {
            ...
            const { data: assignmentData, error: assignmentError } = await this.supabase
                ...
                .single();
            if (assignmentError) {
                return Result.fail(`Failed to find assignment: ${assignmentError.message}`);
            }
            if (!assignmentData) {
            ...
            if (targetsError) {
                return Result.fail(`Failed to find assignment targets: ${targetsError.message}`);
            }
            return this.mapToDomain(assignmentData, targetsData || []);
        }
        catch (error) {
            return Result.fail(`Unexpected error finding assignment: ${error}`);
        }
    }
    async delete(id) {
        try {
            ...
            const { error: targetsError } = await this.supabase
                ...
                .eq('assignment_id', id.toString());
            if (targetsError) {
                return Result.fail(`Failed to delete assignment targets: ${targetsError.message}`);
            }
            // Assignment 삭제
            const { error: assignmentError } = await this.supabase
                .from('learning.assignments')
                ...
                .eq('id', id.toString());
            if (assignmentError) {
                return Result.fail(`Failed to delete assignment: ${assignmentError.message}`);
            }
            return Result.ok();
        }
        catch (error) {
            return Result.fail(`Unexpected error deleting assignment: ${error}`);
        }
    }
    async exists(id) {
        try {
    ...
    async findByTeacherId(teacherId) {
        try {
            ...
            if (assignmentsError) {
                return Result.fail(`Failed to find assignments: ${assignmentsError.message}`);
            }
            const assignments = [];
            ...
            return Result.fail(`Unexpected error finding assignments by teacher: ${error}`);
        }
    }
    async findByProblemSetId(problemSetId) {
        try {
            const { data: assignmentsData, error: assignmentsError } = await this.supabase
                .from('learning.assignments')
                .select('*')
                ...
                .order('created_at', { ascending: false });
            if (assignmentsError) {
                return Result.fail(`Failed to find assignments by problem set: ${assignmentsError.message}`);
            }
            const assignments = [];
            for (const assignmentData of assignmentsData || []) {
                // 각 과제의 배정 대상들 조회
            ...
            return Result.fail(`Unexpected error finding assignments by problem set: ${error}`);
        }
    }
    async findByStatus(status) {
        try {
            const { data: assignmentsData, error: assignmentsError } = await this.supabase
                ...
                .order('created_at', { ascending: false });
            if (assignmentsError) {
                return Result.fail(`Failed to find assignments by status: ${assignmentsError.message}`);
            }
            const assignments = [];
            for (const assignmentData of assignmentsData || []) {
                // 각 과제의 배정 대상들 조회
            ...
            return Result.fail(`Unexpected error finding assignments by status: ${error}`);
        }
    }
    // === 필터링된 조회 ===
    async findWithFilter(filter, sort, page = 1, limit = 20) {
        try {
            ...
            if (assignmentsError) {
                return Result.fail(`Failed to find assignments with filter: ${assignmentsError.message}`);
            }
            const assignments = [];
            for (const assignmentData of assignmentsData || []) {
                // 각 과제의 배정 대상들 조회
            ...
            return Result.fail(`Unexpected error finding assignments with filter: ${error}`);
        }
    }
    async findActiveAssignments(teacherId) {
        try {
            let query = this.supabase
                .from('learning.assignments')
                .select('*')
            ...
            if (assignmentsError) {
                return Result.fail(`Failed to find active assignments: ${assignmentsError.message}`);
            }
            const assignments = [];
            for (const assignmentData of assignmentsData || []) {
                // 각 과제의 배정 대상들 조회
            ...
            return Result.fail(`Unexpected error finding active assignments: ${error}`);
        }
    }
    async findAssignmentsDueSoon(daysFromNow) {
        try {
            const now = new Date();
            ...
            if (assignmentsError) {
                return Result.fail(`Failed to find assignments due soon: ${assignmentsError.message}`);
            }
            const assignments = [];
            for (const assignmentData of assignmentsData || []) {
                // 각 과제의 배정 대상들 조회
            ...
            return Result.fail(`Unexpected error finding assignments due soon: ${error}`);
        }
    }
    async countByTeacherAndStatus(teacherId, status) {
        try {
            const { count, error } = await this.supabase
                ...
                .eq('status', status);
            if (error) {
                return Result.fail(`Failed to count assignments: ${error.message}`);
            }
            return Result.ok(count || 0);
        }
        catch (error) {
            return Result.fail(`Unexpected error counting assignments: ${error}`);
        }
    }
    // === 헬퍼 메서드들 ===
    mapToDomain(record, targets) {
        try {
            // Value Objects 복원
            const dueDateResult = DueDate.create(new Date(record.due_date));
            if (dueDateResult.isFailure) {
                return Result.fail(`Failed to restore due date: ${dueDateResult.error}`);
            }
            // AssignmentTarget 복원
            ...
            if (assignmentResult.isFailure) {
                return Result.fail(`Failed to restore assignment: ${assignmentResult.error}`);
            }
        ...
        try {
            ...
            if (record.target_type === 'class') {
                ...
                if (classIdResult.isFailure) {
                    return Result.fail(`Failed to restore class ID: ${classIdResult.error}`);
                }
                const identifierResult = AssignmentTargetIdentifier.createForClass(classIdResult.value);
                if (identifierResult.isFailure) {
                    return Result.fail(`Failed to restore target identifier: ${identifierResult.error}`);
                }
                ...
                if (studentIdResult.isFailure) {
                    return Result.fail(`Failed to restore student ID: ${studentIdResult.error}`);
                }
                ...
                return Result.fail(`Invalid target type: ${record.target_type}`);
            }
            // AssignmentTarget 복원
            ...
            if (targetResult.isFailure) {
                return Result.fail(`Failed to restore assignment target: ${targetResult.error}`);
            }
            return Result.ok(targetResult.value);
        }
        catch (error) {
            return Result.fail(`Failed to map target record to domain: ${error}`);
        }
    }
    mapSortField(field) {
        switch (field) {
...
//# sourceMappingURL=SupabaseAssignmentRepository.js.map