import { DomainEvent, IEventDispatcher } from '@woodie/domain';
...
// Assignment Domain Events (이벤트들은 도메인에서 정의되어야 함)
interface AssignmentCreatedEvent extends DomainEvent {
...
interface AssignmentActivatedEvent extends DomainEvent {
  eventName: 'AssignmentActivated';
  assignmentId: string;
  teacherId: string;
  activatedAt: Date;
}

interface AssignmentTargetAddedEvent extends DomainEvent {
  eventName: 'AssignmentTargetAdded';
  assignmentId: string;
...
interface AssignmentTargetRevokedEvent extends DomainEvent {
  eventName: 'AssignmentTargetRevoked';
  assignmentId: string;
  targetType: 'class' | 'student';
  targetId: string;
  revokedBy: string;
...
interface AssignmentDueDateChangedEvent extends DomainEvent {
  eventName: 'AssignmentDueDateChanged';
  assignmentId: string;
  teacherId: string;
  oldDueDate: Date;
  newDueDate: Date;
...
interface AssignmentDueDateExtendedEvent extends DomainEvent {
  eventName: 'AssignmentDueDateExtended';
  assignmentId: string;
  teacherId: string;
  oldDueDate: Date;
  newDueDate: Date;
...
interface AssignmentOverdueEvent extends DomainEvent {
  eventName: 'AssignmentOverdue';
  assignmentId: string;
  teacherId: string;
  title: string;
  dueDate: Date;
...
interface AssignmentClosedEvent extends DomainEvent {
  eventName: 'AssignmentClosed';
  assignmentId: string;
  teacherId: string;
  closedAt: Date;
  reason: 'manual' | 'overdue' | 'system';
}
...
export class AssignmentEventHandlers {
  constructor(
    ...
    private cachedAssignmentService: CachedAssignmentService,
  ...
  private async handleAssignmentCreated(event: AssignmentCreatedEvent): Promise<void> {
    try {
      ...
      const assignmentResult = await this.assignmentService['assignmentRepository'].findById(
        { toString: () => event.assignmentId } as any
      );
  ...
  private async handleAssignmentActivated(event: AssignmentActivatedEvent): Promise<void> {
    try {
  ...
  private async handleAssignmentTargetAdded(event: AssignmentTargetAddedEvent): Promise<void> {
    try {
  ...
  private async handleAssignmentTargetRevoked(event: AssignmentTargetRevokedEvent): Promise<void> {
    try {
  ...
  private async handleAssignmentDueDateChanged(event: AssignmentDueDateChangedEvent): Promise<void> {
    try {
      ...
      const notificationResult = await this.notificationService.notifyAssignmentDueDateChanged(
        assignment,
        event.oldDueDate,
        'changed'
      );
  ...
  private async handleAssignmentDueDateExtended(event: AssignmentDueDateExtendedEvent): Promise<void> {
    try {
  ...
  private async handleAssignmentOverdue(event: AssignmentOverdueEvent): Promise<void> {
    try {
  ...
  private async handleAssignmentClosed(event: AssignmentClosedEvent): Promise<void> {
    try {
  ...
  private async invalidateCachesForAssignmentChange(assignmentId: string, teacherId?: string): Promise<void> {
    try {
  ...
  private async invalidateCachesForDueDateChange(assignmentId: string, teacherId?: string): Promise<void> {
    try {
      await this.cachedAssignmentService.invalidateAssignmentCache(assignmentId);
      ...
      await this.cachedAssignmentService['cacheService'].deleteByPattern(
        `${this.cachedAssignmentService['CACHE_PREFIX']}due_soon:*`
      );
      await this.cachedAssignmentService['cacheService'].deleteByPattern(
        `${this.cachedAssignmentService['CACHE_PREFIX']}overdue:*`
      );
