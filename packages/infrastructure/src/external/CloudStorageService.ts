...
export abstract class CloudStorageService {
  ...
  abstract initiateMultipartUpload(
    key: string,
    options?: UploadOptions
  ): Promise<Result<string>>; // uploadId 반환
  ...
  // 폴더 내 모든 파일 삭제
  async deleteFolder(folderPath: string): Promise<Result<{ deletedCount: number; failedDeletes: string[] }>> {
    try {
  ...
  // 파일 크기 및 스토리지 사용량 통계
  async getStorageStatistics(): Promise<Result<{
    totalFiles: number;
    totalSize: number;
    averageFileSize: number;
    filesByType: Record<string, number>;
    sizeByType: Record<string, number>;
    oldestFile?: { key: string; lastModified: Date };
    newestFile?: { key: string; lastModified: Date };
  }>> {
    try {
  ...
  ): Promise<Result<Array<{ operation: any; success: boolean; error?: string; result?: any }>>> {
    try {
        ...
        operations.map(async (operation) => {
          switch (operation.type) {
            case 'upload':
              ...
              return await this.uploadFile(operation.key, operation.data, operation.options as UploadOptions);

            case 'download':
              return await this.downloadFile(operation.key, operation.options as DownloadOptions);
            ...
            case 'copy':
              ...
              return await this.copyFile(operation.key, operation.destinationKey, operation.options as UploadOptions);
  ...
  async backupFile(
    key: string,
    backupStorageService: CloudStorageService,
    backupKey?: string
  ): Promise<Result<void>> {
    try {
      // 원본 파일 다운로드
