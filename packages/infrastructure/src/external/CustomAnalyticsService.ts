...
export class CustomAnalyticsService extends AnalyticsService {
  ...
  async query(query: AnalyticsQuery): Promise<Result<AnalyticsResult>> {
    try {
      // 쿼리 조건에 따라 이벤트 필터링
    ...
    try {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        const stepEvents = this.analyticsEvents.filter(event => {
          if (event.timestamp < fromDate || event.timestamp > toDate) return false;
          if (event.eventName !== step.eventName) return false;
          if (!event.userId) return false;

          // 첫 번째 단계가 아니라면 이전 단계를 완료한 사용자만
          if (i > 0 && !previousStepUsers.has(event.userId)) return false;
  ...
  // 실시간 분석 대시보드 데이터
  async getRealTimeAnalytics(): Promise<Result<{
  ...
  ): Array<{ dimensions: Record<string, string>; metrics: Record<string, number>; timestamp?: Date }> {
    ...
    const results = Array.from(groups.entries()).map(([groupKey, groupEvents]) => {
      ...
      switch (query.aggregation) {
        ...
        case 'unique':
          ...
          break;
        case 'sum':
          metrics.sum = groupEvents.reduce((sum, e) => 
            sum + (typeof e.properties.value === 'number' ? e.properties.value : 0), 0
          );
          break;
        case 'avg':
  ...
  private calculateUserFlow(events: AnalyticsEvent[]): Array<{ from: string; to: string; count: number }> {
    const transitions = new Map<string, number>();
    
    // 사용자별 이벤트 시퀀스 생성
    const userEventSequences = new Map<string, string[]>();
  ...
  private getPeriodMilliseconds(period: 'daily' | 'weekly' | 'monthly'): number {
    switch (period) {
      case 'daily':
  ...
  private getRetentionPeriods(period: 'daily' | 'weekly' | 'monthly'): number[] {
