...
import { DataMaskingService, MaskingContext } from './DataMaskingService';
...
export class SecurityMiddleware {
  ...
  private readonly loginAttempts: Map<string, { count: number; lockedUntil?: Date }> = new Map();
  ...
  // 교사 권한 검증 미들웨어
  requireTeacherAuth(resource?: string, action: string = 'read') {
    return async (req: SecurityRequest, res: Response, next: NextFunction) => {
      try {
        ...
        if (resource) {
          const authResult = await this.authorizationService.authorizeTeacherAccess(
            req.authContext!,
  ...
  // 학생 권한 검증 미들웨어
  requireStudentAuth(action: string = 'read') {
    return async (req: SecurityRequest, res: Response, next: NextFunction) => {
      try {
        ...
        const authResult = await this.authorizationService.authorizeStudentAccess(
          req.authContext!,
  ...
  // 데이터 마스킹 미들웨어 (응답 데이터)
  dataMaskingMiddleware(policyName: string = 'default') {
    return async (req: SecurityRequest, res: Response, next: NextFunction) => {
      ...
      res.json = (data: any) => {
        try {
          const maskingContext: MaskingContext = {
  ...
  private setSecurityMetadata(req: SecurityRequest): void {
  ...
  private checkRateLimit(req: SecurityRequest): { allowed: boolean; remaining: number; resetTime: Date } {
  ...
  private checkCORS(req: SecurityRequest): { allowed: boolean } {
  ...
  private checkCSRF(req: SecurityRequest): { valid: boolean } {
  ...
  private async validateSession(req: SecurityRequest): Promise<Result<void>> {
    const sessionId = req.get('X-Session-Id') || req.sessionId;
    
    if (!sessionId) {
      return Result.fail<void>('No session ID provided');
    }
  ...
  private recordFailedLogin(identifier: string): void {
  ...
  private handleCORSViolation(req: SecurityRequest, res: Response): Response {
  ...
  private handleCSRFViolation(req: SecurityRequest, res: Response): Response {
  ...
  private handleInvalidSession(req: SecurityRequest, res: Response, error: string): Response {
  ...
  private handleAuthorizationFailure(req: SecurityRequest, res: Response, error: string): Response {
