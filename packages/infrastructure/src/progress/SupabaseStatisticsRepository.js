...
export class SupabaseStatisticsRepository {
     ...
     */
    async findByStudentAndProblemSet(studentId, problemSetId) {
        try {
            ...
            if (error) {
                ...
                return Result.fail(`Failed to find statistics: ${error.message}`);
            }
            if (!data) {
                return Result.ok(null);
            }
            ...
            return Result.fail(`Unexpected error finding statistics: ${err}`);
        }
    }
    /**
     * 학생의 모든 문제집 통계 조회
    ...
    async findByStudentId(studentId) {
        try {
            ...
            if (error) {
                return Result.fail(`Failed to find statistics by student: ${error.message}`);
            }
            if (!data) {
                return Result.ok([]);
            }
            ...
            if (failures.length > 0) {
                return Result.fail(`Failed to convert some statistics: ${failures[0].error}`);
            }
            const statistics = domainResults.map(result => result.value);
            return Result.ok(statistics);
        }
        catch (err) {
            return Result.fail(`Unexpected error finding statistics by student: ${err}`);
        }
    }
    /**
     * 문제집별 모든 학생 통계 조회
    ...
    async findByProblemSetId(problemSetId) {
        try {
            const { data, error } = await this.client
                ...
                .order('created_at', { ascending: false });
            if (error) {
                return Result.fail(`Failed to find statistics by problem set: ${error.message}`);
            }
            if (!data) {
                return Result.ok([]);
            }
            ...
            return Result.fail(`Unexpected error finding statistics by problem set: ${err}`);
        }
    }
    /**
     * 통계 저장 (생성 또는 업데이트)
    ...
    async save(statistics) {
        try {
            ...
            if (error) {
                return Result.fail(`Failed to save statistics: ${error.message}`);
            }
            return Result.ok();
        }
        catch (err) {
            return Result.fail(`Unexpected error saving statistics: ${err}`);
        }
    }
    /**
     * 통계 삭제
     */
    async delete(statisticsId) {
        try {
            const { error } = await this.client
                .from(`${this.schema}.${this.tableName}`)
                .delete()
            ...
            if (error) {
                return Result.fail(`Failed to delete statistics: ${error.message}`);
            }
            return Result.ok();
        }
        catch (err) {
            return Result.fail(`Unexpected error deleting statistics: ${err}`);
        }
    }
    /**
     * 클래스별 통계 조회
     */
    async findByClassId(classId, problemSetId) {
        try {
        ...
        `)
            ...
            if (error) {
                return Result.fail(`Failed to find statistics by class: ${error.message}`);
            }
            if (!data) {
                return Result.ok([]);
            }
            ...
            return Result.fail(`Unexpected error finding statistics by class: ${err}`);
        }
    }
    /**
     * 완료율 기준 상위 학생 조회
     */
    async findTopByCompletionRate(problemSetId, limit = 10) {
        try {
            const { data, error } = await this.client
                .from(`${this.schema}.${this.tableName}`)
                .select('*')
                ...
                .limit(limit);
            if (error) {
                return Result.fail(`Failed to find top by completion rate: ${error.message}`);
            }
            if (!data) {
                return Result.ok([]);
            }
            ...
            return Result.ok(statistics.slice(0, limit));
        }
        catch (err) {
            return Result.fail(`Unexpected error finding top by completion rate: ${err}`);
        }
    }
    /**
     * 정답률 기준 상위 학생 조회
     */
    async findTopByAccuracyRate(problemSetId, limit = 10) {
        try {
            const { data, error } = await this.client
                .from(`${this.schema}.${this.tableName}`)
                .select('*')
            ...
            if (error) {
                return Result.fail(`Failed to find top by accuracy rate: ${error.message}`);
            }
            if (!data) {
                return Result.ok([]);
            }
            ...
            return Result.fail(`Unexpected error finding top by accuracy rate: ${err}`);
        }
    }
    /**
     * 특정 기간 내 생성된 통계 조회
    ...
    async findByDateRange(startDate, endDate) {
        try {
            const { data, error } = await this.client
                ...
                .order('created_at', { ascending: false });
            if (error) {
                return Result.fail(`Failed to find statistics by date range: ${error.message}`);
            }
            if (!data) {
                return Result.ok([]);
            }
            ...
            return Result.fail(`Unexpected error finding statistics by date range: ${err}`);
        }
    }
    /**
     * 문제집별 평균 통계 계산
     */
    async calculateAverageStatistics(problemSetId) {
        try {
...
//# sourceMappingURL=SupabaseStatisticsRepository.js.map