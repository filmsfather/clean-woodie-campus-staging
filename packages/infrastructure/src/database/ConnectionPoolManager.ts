...
export class ConnectionPoolManager {
  ...
  private readonly healthCheckHistory: HealthCheckResult[] = [];
  ...
  async initialize(): Promise<Result<void>> {
    try {
      ...
      if (!healthCheck.isHealthy) {
        return Result.fail<void>('Initial health check failed: ' + healthCheck.error);
      }
      ...
      return Result.fail<void>('Connection pool initialization failed');
    }
  }
  ...
  async acquireConnection<T>(
    ...
    try {
      ...
      return Result.fail<T>(`Connection operation failed: ${error instanceof Error ? error.message : String(error)}`);
  ...
  private generateRecommendations(): string[] {
    ...
    if (successRate < 95) {
      recommendations.push(
        `Low connection success rate (${successRate.toFixed(1)}%). Consider increasing retry attempts or connection timeout.`
      );
    }
    ...
    if (this.metrics.averageConnectionTime > 5000) {
      recommendations.push(
        `High average connection time (${this.metrics.averageConnectionTime.toFixed(0)}ms). Consider optimizing queries or increasing connection pool size.`
      );
    }
    ...
    if (this.metrics.waitingRequests > this.config.maxConnections * 0.5) {
      recommendations.push(
        `High number of waiting requests (${this.metrics.waitingRequests}). Consider increasing max connections from ${this.config.maxConnections}.`
      );
    }
    ...
    if (this.metrics.peakConnections > this.config.maxConnections * 0.8) {
      recommendations.push(
        `Peak connections (${this.metrics.peakConnections}) approaching max limit (${this.config.maxConnections}). Consider increasing pool size.`
      );
    }
    ...
    if (unhealthyCount > 2) {
      recommendations.push(
        `Frequent health check failures detected (${unhealthyCount}/5). Check database connectivity and performance.`
      );
    }
