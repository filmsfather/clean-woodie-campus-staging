 ...
 */
export class NotificationStatisticsService implements INotificationStatisticsService {
  constructor(
  ...
  async getStatistics(
    ...
    try {
      ...
      if (notificationsResult.isFailure) {
        return Result.fail<NotificationStatistics>(`Failed to get notifications: ${notificationsResult.error}`)
      }
      ...
      return Result.fail<NotificationStatistics>(`Failed to calculate notification statistics: ${error}`)
    }
  }
   ...
   */
  private async getNotificationsInPeriod(
    studentId: UniqueEntityID,
    fromDate: Date,
    toDate: Date
  ): Promise<Result<any[]>> {
    try {
      ...
      return Result.fail<any[]>(`Failed to get notifications for period: ${error}`)
    }
  }
  ...
  private async calculateSystemStats(fromDate: Date, toDate: Date): Promise<any> {
    // 시스템 전체 통계 계산 로직
   ...
   * 실시간 통계 (최근 24시간)
   */
  async getRealtimeStatistics(): Promise<Result<{
 ...
 */
export class MockNotificationStatisticsService implements INotificationStatisticsService {
  private mockData: NotificationStatistics
  ...
  async getStatistics(
    ...
    return Result.ok<NotificationStatistics>(this.mockData)
  }
