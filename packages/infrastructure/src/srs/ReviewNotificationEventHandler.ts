...
export class ReviewNotificationEventHandler {
   ...
   */
  async handle(event: ReviewNotificationScheduledEvent): Promise<void> {
    try {
   ...
   * 현재 구현에서는 로깅만 수행
   */
  private async scheduleDelayedNotification(event: ReviewNotificationScheduledEvent): Promise<void> {
   ...
   */
  private generateNotificationContent(event: ReviewNotificationScheduledEvent): {
    title: string
    body: string
  } {
