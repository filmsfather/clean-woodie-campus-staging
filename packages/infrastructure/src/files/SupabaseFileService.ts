...
} from '@woodie/application/common/interfaces/IFileService';
...
export interface SupabaseFileConfig {
  ...
  allowedTypes?: string[];
  urlExpirationTime?: number; // seconds
...
export class SupabaseFileService implements IFileService {
  ...
  private readonly config: SupabaseFileConfig;

  constructor(
    supabase: SupabaseClient,
    logger: ILogger,
    config: SupabaseFileConfig
  ) {
  ...
  async getFileMetadata(fileId: string): Promise<Result<FileMetadata>> {
    try {
  ...
  async fileExists(fileId: string): Promise<Result<boolean>> {
    try {
      const metadataResult = await this.getFileMetadata(fileId);
  ...
  async readTextFile(fileId: string, encoding = 'utf-8'): Promise<Result<string>> {
    try {
      ...
      const text = bufferResult.value.toString(encoding as BufferEncoding);
  ...
  async readJsonFile<T>(fileId: string): Promise<Result<T>> {
    try {
  ...
  async cleanupTempFiles(olderThanHours = 24): Promise<Result<number>> {
    try {
  ...
  private generateFileName(originalName: string, options?: FileUploadOptions): string {
  ...
  private getMimeType(file: Buffer | Blob | File, fileName: string): string {
    if (typeof File !== 'undefined' && file instanceof File) {
      return file.type;
    }
    if (typeof Blob !== 'undefined' && file instanceof Blob) {
  ...
  private async saveFileMetadata(metadata: FileMetadata): Promise<Result<void>> {
    try {
  ...
  private mapToFileMetadata(data: any): FileMetadata {
    return {
      id: data.id,
