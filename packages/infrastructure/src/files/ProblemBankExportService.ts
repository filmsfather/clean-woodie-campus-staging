...
import {
  IProblemBankExportService,
  ProblemBankExportOptions,
  ProblemBankImportOptions,
  ExportResult,
  ImportResult,
  ImportValidationResult
} from '@woodie/application/problems/interfaces/IProblemBankExportService';
...
export class ProblemBankExportService implements IProblemBankExportService {
  ...
  private readonly importJobs = new Map<string, any>();
  ...
  async exportProblemBank(
    ...
    options: ProblemBankExportOptions = { format: 'json' }
  ...
  async validateImportFile(
    ...
    try {
      ...
      switch (fileFormat) {
        case 'json':
          const jsonResult = await this.fileService.readJsonFile<ProblemExportData[]>(fileId);
  ...
  async generateExportTemplate(
    ...
    try {
      const templateData: ProblemExportData[] = [];
  ...
  async getExportStatus(exportId: string): Promise<Result<any>> {
    const job = this.exportJobs.get(exportId);
  ...
  async getImportStatus(importId: string): Promise<Result<any>> {
  ...
  async getExportHistory(teacherId: string, limit = 10): Promise<Result<ExportResult[]>> {
    try {
  ...
  async getImportHistory(teacherId: string, limit = 10): Promise<Result<ImportResult[]>> {
    try {
      const { data, error } = await this.supabase
        .from('import_history')
        .select('*')
  ...
  private async performExport(
    ...
    try {
      // 문제 조회
      const problemsResult = await this.problemRepository.findByTeacherId(
        teacherId,
        {
      ...
      for (let i = 0; i < problems.length; i++) {
        ...
        const problemData: ProblemExportData = {
          id: (options as any).preserveIds ? problem.id.toString() : undefined,
    ...
    try {
      ...
      for (let i = 0; i < importData.length; i += batchSize) {
        ...
        for (const [index, data] of batch.entries()) {
          ...
          try {
            const importResult = await this.importSingleProblem(
              teacherId,
              data,
              options
            );
  ...
  private async importSingleProblem(
    teacherId: string,
    data: ProblemExportData,
    options: ProblemBankImportOptions
  ...
  // 헬퍼 메서드들
  private detectFormat(mimeType: string): 'json' | 'csv' | 'xlsx' {
    switch (mimeType) {
  ...
  private async parseCsvFile(fileId: string): Promise<Result<ProblemExportData[]>> {
    // CSV 파싱 로직 구현
    return Result.fail('CSV parsing not implemented');
  }

  private async parseXlsxFile(fileId: string): Promise<Result<ProblemExportData[]>> {
    // XLSX 파싱 로직 구현
    return Result.fail('XLSX parsing not implemented');
  }

  private validateImportData(data: any[]): { errors: any[], warnings: string[] } {
    const errors: any[] = [];
  ...
  private generateCsvBuffer(data: ProblemExportData[]): Buffer {
    // CSV 생성 로직
  ...
  private generateXlsxBuffer(data: ProblemExportData[]): Buffer {
  ...
  private calculateEstimatedTime(job: ExportJob | any): number {
  ...
  private mapToExportResult(data: any): ExportResult {
    return {
      fileId: data.file_id,
  ...
  private mapToImportResult(data: any): ImportResult {
    return {
