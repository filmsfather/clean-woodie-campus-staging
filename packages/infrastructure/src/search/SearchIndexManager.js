...
export class SearchIndexManager {
     ...
     * 검색 인덱스 생성 또는 업데이트
     */
    async createOrUpdateSearchIndex() {
        try {
     ...
     * 검색 성능 분석
     */
    async analyzeSearchPerformance(sampleQueries) {
        try {
            const results = [];
     ...
     * 사용자 정의 검색 설정 적용
     */
    async applyCustomSearchConfig(config) {
        try {
    ...
    async ensureSearchVectorColumn() {
    ...
    async createSearchVectorUpdateFunction() {
        const functionSQL = `
    ...
    async createSearchVectorTrigger() {
        const triggerSQL = `
    ...
    async updateExistingSearchVectors() {
    ...
    async updateSynonymDictionary(synonyms) {
    ...
    async updateLanguageConfig(language) {
