...
export class LoggingUtils {
  ...
  static queryToContext(query: string, params?: any[], duration?: number): LogContext {
    ...
    if (params && params.length > 0) {
      context.params = params.map(param => 
        typeof param === 'string' && this.isSensitiveData(param) 
  ...
  static buildMetadata(options: {
    ...
    if (options.correlationId) metadata.correlationId = options.correlationId;
   ...
   */
  static logExecutionTime<T extends (...args: any[]) => any>(
    target: T,
    operation: string,
    logger: any
  ): T {
    return ((...args: any[]) => {
      ...
      try {
        ...
        const duration = Date.now() - startTime;
        logger.error(`Failed ${operation}`, this.errorToContext(error as Error), {
          correlationId,
          operation,
          duration
        });
