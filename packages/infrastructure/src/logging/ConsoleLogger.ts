...
export class ConsoleLogger implements ILogger {
  ...
  constructor(logLevel: LogLevel = LogLevel.INFO, parentContext?: LogContext) {
  ...
  private formatMessage(coloredLevel: string, message: string, logData: any): string {
  ...
  private extractMetadata(metadata?: LogMetadata): Record<string, any> {
    if (!metadata) return {};
    
    const extracted: Record<string, any> = {};
    
    if (metadata.correlationId) extracted.correlationId = metadata.correlationId;
    if (metadata.userId) extracted.userId = metadata.userId;
    if (metadata.sessionId) extracted.sessionId = metadata.sessionId;
    if (metadata.requestId) extracted.requestId = metadata.requestId;
    if (metadata.operation) extracted.operation = metadata.operation;
    if (metadata.duration !== undefined) extracted.duration = metadata.duration;
