...
export class SupabaseEventStore {
    ...
    async getAllEvents(fromEventId, limit = 100) {
        try {
            let query = this.supabase
                .from(this.config.tableName)
                .select('*')
    ...
    // 이벤트 타입별 조회
    async getEventsByType(eventType, limit = 100, fromDate) {
        try {
            let query = this.supabase
                .from(this.config.tableName)
                .select('*')
    ...
    async getEventStream(options = {}) {
        try {
            const { fromEventId, toEventId, eventTypes, aggregateIds, fromDate, toDate, limit = 100, offset = 0 } = options;
            let query = this.supabase
                .from(this.config.tableName)
                .select('*', { count: 'exact' })
                .order('created_at', { ascending: true })
    ...
    async getEventStatistics(options = {}) {
        try {
            const { fromDate, toDate, eventTypes, aggregateIds } = options;
            let query = this.supabase
                .from(this.config.tableName)
    ...
    async considerSnapshot(aggregateId) {
        if (!this.config.enableSnapshots) {
            return;
        }
        try {
...
//# sourceMappingURL=SupabaseEventStore.js.map