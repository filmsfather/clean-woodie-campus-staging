...
export class UsageStatisticsService {
    ...
    usageEvents = [];
    retentionPeriodMs;
    constructor(logger, metricsCollector, retentionPeriodMs = 90 * 24 * 60 * 60 * 1000 // 90일
    ) {
        this.logger = logger;
    ...
    // 문제 사용 통계
    async getProblemUsageStats(problemId, fromDate, toDate) {
        try {
            ...
            // 통계 완성
            const completedStats = Array.from(problemStats.values()).map(stats => ({
                ...stats,
                completionRate: stats.totalAttempts > 0 ? (stats.totalCompletions / stats.totalAttempts) * 100 : 0,
                popularityScore: this.calculatePopularityScore(stats),
                difficultyRating: this.calculateDifficultyRating(stats)
            }));
            ...
            return Result.fail('Failed to get problem usage stats');
        }
    }
    // 교사 사용 통계
    async getTeacherUsageStats(teacherId, fromDate, toDate) {
        try {
            const events = this.getFilteredEvents({
                userId: teacherId,
                userRole: 'teacher',
            ...
            for (const [teacherId, stats] of teacherStats.entries()) {
                ...
                stats.teachingEffectiveness = this.calculateTeachingEffectiveness(stats);
                stats.studentEngagement = this.calculateStudentEngagement(stats);
                stats.contentCreationRate = this.calculateContentCreationRate(stats);
            }
            return Result.ok(Array.from(teacherStats.values()));
        }
        catch (error) {
            ...
            return Result.fail('Failed to get teacher usage stats');
        }
    }
    // 학생 사용 통계
    async getStudentUsageStats(studentId, fromDate, toDate) {
        try {
            const events = this.getFilteredEvents({
                userId: studentId,
                userRole: 'student',
            ...
            return Result.ok(Array.from(studentStats.values()));
        }
        catch (error) {
            ...
            return Result.fail('Failed to get student usage stats');
        }
    }
    // 시스템 전체 사용 통계
    async getSystemUsageStats(fromDate, toDate) {
        try {
            const now = new Date();
            ...
            // 총 리소스 계산
            const problemIds = new Set(events
                .filter(e => e.resourceType === 'problem')
                .map(e => e.resourceId));
            const problemSetIds = new Set(events
            ...
            return Result.fail('Failed to get system usage stats');
        }
    }
    // 실시간 사용자 활동 추적
    trackUserActivity(userId, activity, metadata = {}) {
        ...
        // 실시간 메트릭 업데이트
        this.metricsCollector.setGauge('active_users_current', this.getCurrentActiveUsers(), { activity_type: activity });
    }
    // 성능 메트릭 기록
    recordPerformanceMetric(operationType, durationMs, success, metadata = {}) {
        this.metricsCollector.recordDuration('operation_duration', durationMs, {
            operation: operationType,
            success: success.toString(),
            ...metadata
        });
        if (!success) {
            this.metricsCollector.incrementCounter('operation_errors_total', {
                operation: operationType,
    ...
    calculateDifficultyRating(stats) {
    ...
    calculateFavoriteSubjects(studentId) {
    ...
    calculateStrongAreas(studentId) {
        // 높은 점수를 받은 영역들
        return [];
    }
    calculateImprovementAreas(studentId) {
        // 낮은 점수를 받은 영역들
        return [];
    }
    calculateProgressTrend(studentId) {
        // 최근 성과 변화 추이 분석
        return 'stable';
    }
    getUniqueUsersByPeriod(events, period) {
        const now = new Date();
        let cutoffTime;
        switch (period) {
            case 'daily':
    ...
    calculatePopularFeatures(events) {
    ...
    getCurrentActiveUsers() {
        ...
        const activeUsers = new Set(this.usageEvents
