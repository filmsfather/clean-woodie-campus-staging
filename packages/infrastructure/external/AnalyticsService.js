...
export class AnalyticsService {
    ...
    // 교육 플랫폼 특화 메서드들
    // 학습 진도 추적
    async trackLearningProgress(studentId, courseId, lessonId, progressPercentage, timeSpent, context) {
    ...
    // 문제 풀이 추적
    async trackProblemAttempt(studentId, problemId, attempt, context) {
    ...
    // 교사 활동 추적
    async trackTeacherActivity(teacherId, activity, details, context) {
    ...
    // 학습 세션 시작
    async startLearningSession(studentId, sessionData) {
    ...
    // 학습 세션 종료
    async endLearningSession(studentId, sessionId, sessionSummary) {
    ...
    // 학습 성과 분석
    async analyzeLearningPerformance(studentId, fromDate, toDate) {
        try {
            // 학습 진도 쿼리
            const progressQuery = {
            ...
            const totalStudyTime = sessionData.reduce((sum, session) => sum + (session.metrics.session_duration_minutes || 0), 0);
            const problemsSolved = problemData.reduce((sum, problem) => sum + (problem.metrics.count || 0), 0);
    ...
    // 교사 대시보드 분석
    async analyzeTeacherDashboard(teacherId, fromDate, toDate) {
        try {
            // 교사 활동 쿼리
            const teacherQuery = {
    ...
    // 시스템 사용량 분석
    async analyzeSystemUsage(fromDate, toDate) {
        try {
            // 일별 활성 사용자 쿼리
            const dauQuery = {
                fromDate,
    ...
    calculateAccuracyRate(problemData) {
        const totalAttempts = problemData.reduce((sum, p) => sum + (p.metrics.count || 0), 0);
        const correctAttempts = problemData.reduce((sum, p) => sum + (p.metrics.correct_answers || 0), 0);
        return totalAttempts > 0 ? (correctAttempts / totalAttempts) * 100 : 0;
    }
    analyzeSubjectPerformance(problemData) {
    ...
    calculateProgressTrend(progressData) {
        ...
        const firstHalfAvg = firstHalf.reduce((sum, p) => sum + (p.metrics.avg || 0), 0) / firstHalf.length;
        const secondHalfAvg = secondHalf.reduce((sum, p) => sum + (p.metrics.avg || 0), 0) / secondHalf.length;
    ...
    calculateClassEngagement(studentData) {
    ...
    categorizeStudents(studentData) {
