...
export class SupabaseStorageService extends CloudStorageService {
    supabase;
    constructor(logger, config, supabaseClient) {
        super(logger, config);
    ...
    async uploadFile(key, fileBuffer, options = {}) {
        try {
    ...
    async generateSignedUrl(key, expirationSeconds, method = 'GET') {
        try {
            const normalizedKey = this.normalizeKey(key);
            if (method !== 'GET') {
    ...
    // 파일 변환 (이미지 리사이징 등)
    async getTransformedUrl(key, transformations) {
        try {
            const normalizedKey = this.normalizeKey(key);
