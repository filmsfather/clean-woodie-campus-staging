...
export class SupabaseNotificationSettingsRepository {
    ...
    async findByStudentId(studentId) {
        try {
            ...
            if (error) {
                ...
                return Result.fail(`Failed to find notification settings: ${error.message}`);
            }
            if (!data) {
                return Result.ok(null);
            }
            const domainSettings = this.toDomain(data);
            return Result.ok(domainSettings);
        }
        catch (error) {
            console.error(`Error finding notification settings for student ${studentId.toString()}:`, error);
            return Result.fail(`Unexpected error finding notification settings: ${error}`);
        }
    }
    /**
    ...
    async save(userId, settings) {
        try {
            ...
            return Result.fail(`Failed to save notification settings: ${error}`);
        }
    }
    /**
     * 알림 설정 저장 (생성 또는 업데이트)
    ...
    async delete(userId) {
        try {
            ...
            return Result.fail(`Failed to delete notification settings: ${error}`);
        }
    }
    /**
     * 알림 설정 삭제
