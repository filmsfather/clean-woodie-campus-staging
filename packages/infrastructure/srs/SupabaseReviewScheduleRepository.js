...
export class SupabaseReviewScheduleRepository {
    client;
    ...
    async findOverdueByStudentId(studentId, currentDate) {
        const date = currentDate || new Date();
        return this.findOverdueReviews(studentId, date);
    }
    async findOverdueSchedules(currentDate) {
        const date = currentDate || new Date();
    ...
    async countOverdueByStudentId(studentId, currentDate) {
        const date = currentDate || new Date();
        const yesterdayEnd = new Date(date);
        yesterdayEnd.setDate(yesterdayEnd.getDate() - 1);
        yesterdayEnd.setHours(23, 59, 59, 999);
