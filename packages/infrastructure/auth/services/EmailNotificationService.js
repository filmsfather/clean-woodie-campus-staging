...
export class EmailNotificationService {
    emailAdapter;
    baseUrl;
    constructor(emailAdapter, baseUrl // 프론트엔드 기본 URL (환경변수에서 주입)
    ) {
        this.emailAdapter = emailAdapter;
        this.baseUrl = baseUrl;
    }
    async notifyInviteCreated(invite) {
        try {
            // 초대 링크 생성
            ...
            if (sendResult.isFailure) {
                return Result.fail(`Failed to send invite email: ${sendResult.errorValue}`);
            }
            return Result.ok();
        }
        catch (error) {
            return Result.fail(`Unexpected error sending invite notification: ${error}`);
        }
    }
    async notifyInviteUsed(invite) {
        // 초대 사용 알림 (관리자에게 발송)
        // 현재는 간단히 성공 반환 (YAGNI)
        return Result.ok();
    }
    async notifyInviteExpired(invite) {
        // 초대 만료 알림 (배치 작업용)
        // 현재는 간단히 성공 반환 (YAGNI)
        return Result.ok();
    }
    // 초대 이메일 템플릿 생성 (private 메서드)
    generateInviteEmailContent(data) {
