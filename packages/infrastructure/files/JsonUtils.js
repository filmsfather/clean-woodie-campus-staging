...
export class JsonUtils {
        ...
        try {
            ...
            if (options.strict) {
                ...
                if (!validationResult.isValid) {
                    return Result.fail(`JSON validation failed: ${validationResult.errors.map(e => e.message).join(', ')}`);
                }
            }
            // 날짜 필드 자동 변환
     ...
     * JSON 데이터 검증
     */
    static validate(data, validationOptions) {
        const errors = [];
        const warnings = [];
        // 규칙 기반 검증
    ...
    static removeJsonComments(jsonString) {
        // 간단한 주석 제거 (정교하지 않음)
        return jsonString
            .replace(/\/\*[\s\S]*?\*\//g, '') // 블록 주석
            .replace(/\/\/.*$/gm, ''); // 라인 주석
    }
    static validateJsonStructure(data, options) {
        const errors = [];
        const warnings = [];
        // 최대 깊이 검증
        if (options.maxDepth) {
    ...
    static convertDateFields(data, dateFields) {
    ...
    static filterFields(data, options) {
        ...
        if (data && typeof data === 'object') {
            ...
            for (const [key, value] of Object.entries(data)) {
                const shouldInclude = (!options.includeFields || options.includeFields.includes(key)) &&
    ...
    static convertDateFormat(data, format) {
    ...
    static calculateDepth(data, current = 0) {
        if (data && typeof data === 'object') {
            if (Array.isArray(data)) {
...
export class CsvUtils {
    static parse(csvString, options = {}) {
        try {
            const { delimiter = ',', quote = '"', headers = true, skipEmptyLines = true } = options;
    ...
    static stringify(data, options = {}) {
        try {
            const { delimiter = ',', quote = '"', headers = true, headerRow } = options;
            const lines = [];
            if (data.length === 0) {
                return Result.ok('');
            }
            ...
            for (const item of data) {
                const values = Array.isArray(item)
                    ? item
                    : Object.values(item);
                const escapedValues = values.map(value => this.escapeCsvValue(String(value || ''), delimiter, quote));
    ...
    static escapeCsvValue(value, delimiter, quote) {
