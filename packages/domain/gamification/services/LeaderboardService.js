...
export class LeaderboardService {
    ...
    clock;
    constructor(leaderboardRepository, clock) {
     ...
     * 리더보드를 조회합니다
     */
    async getLeaderboard(type, limit = 10, periodStart, periodEnd) {
        const entriesResult = await this.leaderboardRepository.getLeaderboard(type, limit, periodStart, periodEnd);
     ...
     */
    async getStudentRankInfo(studentId, type, periodStart, periodEnd) {
        const entryResult = await this.leaderboardRepository.getStudentRank(studentId, type, periodStart, periodEnd);
        ...
        const fullLeaderboardResult = await this.leaderboardRepository.getLeaderboard(type, 1000, // 충분히 큰 수
     ...
     */
    async refreshLeaderboard(type, limit = 100) {
        let rawDataResult;
        // 타입에 따라 데이터 조회 방식 결정
        switch (type.value) {
            ...
            case LeaderboardTypeEnum.WEEKLY_TOKENS:
                ...
                rawDataResult = await this.leaderboardRepository.getTokenEarnedByPeriodData(weekStart, weekEnd, limit);
                break;
            case LeaderboardTypeEnum.MONTHLY_TOKENS:
                const monthStart = this.getMonthStart();
                const monthEnd = this.getMonthEnd();
                rawDataResult = await this.leaderboardRepository.getTokenEarnedByPeriodData(monthStart, monthEnd, limit);
                break;
            default:
     ...
     * 주요 리더보드들을 한번에 조회합니다
     */
    async getMainLeaderboards(limit = 10) {
        const tokenBalanceResult = await this.getLeaderboard((await LeaderboardType.tokenBalance()).getValue(), limit);
        const tokenEarnedResult = await this.getLeaderboard((await LeaderboardType.tokenEarned()).getValue(), limit);
        const achievementsResult = await this.getLeaderboard((await LeaderboardType.achievements()).getValue(), limit);
        const weeklyTokensResult = await this.getLeaderboard((await LeaderboardType.weeklyTokens()).getValue(), limit, this.getWeekStart(), this.getWeekEnd());
