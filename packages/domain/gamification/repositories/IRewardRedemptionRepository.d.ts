...
export interface IRewardRedemptionRepository {
     ...
     * 학생의 교환 기록 조회
     */
    findByStudentId(studentId: StudentId, status?: RedemptionStatus, limit?: number): Promise<Result<RewardRedemption[]>>;
    /**
     * 특정 보상의 교환 기록 조회
     */
    findByRewardId(rewardId: UniqueEntityID, status?: RedemptionStatus, limit?: number): Promise<Result<RewardRedemption[]>>;
    /**
     * 상태별 교환 기록 조회
     */
    findByStatus(status: RedemptionStatus, limit?: number): Promise<Result<RewardRedemption[]>>;
    /**
     * 교환 기록 저장
     */
     ...
     * 기간별 교환 통계 조회
     */
    getRedemptionStatsByPeriod(startDate: Date, endDate: Date): Promise<Result<{
        totalRedemptions: number;
        totalTokensSpent: number;
     ...
     * 오래된 처리 대기 교환들 조회
     */
    findOldPendingRedemptions(olderThanHours: number, limit?: number): Promise<Result<RewardRedemption[]>>;
}
//# sourceMappingURL=IRewardRedemptionRepository.d.ts.map