 ...
 */
export class StudyStreak extends AggregateRoot {
     ...
     */
    updateStreakInternal(updateProps) {
    ...
    recordStudy(studyDate) {
        ...
        if (lastStudyDateOnly.getTime() === yesterday.getTime()) {
            ...
            // 스트릭 달성 이벤트 발행
            this.addDomainEvent(new StreakAchievedEvent(this.props.studentId, previousStreak, newCurrentStreak, newCurrentStreak === newLongestStreak, this.getStreakMilestone(newCurrentStreak)));
        }
        else if (lastStudyDateOnly.getTime() < yesterday.getTime()) {
            // 스트릭 끊김: 기존 스트릭이 있었다면 스트릭 잃음 이벤트 발행
            if (this.props.currentStreak > 0) {
                const daysSinceLastStudy = Math.floor((studyDateOnly.getTime() - lastStudyDateOnly.getTime()) / (24 * 60 * 60 * 1000));
                this.addDomainEvent(new StreakLostEvent(this.props.studentId, this.props.currentStreak, this.props.longestStreak, this.props.lastStudyDate, daysSinceLastStudy));
            }
            // 새로 시작 (1일부터)
            this.updateStreakInternal({
                currentStreak: 1,
            ...
            // 새로운 스트릭 시작 이벤트 발행
            this.addDomainEvent(new StreakAchievedEvent(this.props.studentId, 0, 1, false));
        }
    }
    /**
     * 스트릭 리셋
    ...
    static create(props, id) {
        ...
        if (props.currentStreak < 0) {
            return Result.fail('현재 스트릭은 음수가 될 수 없습니다');
        }
        if (props.longestStreak < 0) {
            return Result.fail('최장 스트릭은 음수가 될 수 없습니다');
        }
        if (props.longestStreak < props.currentStreak) {
            return Result.fail('최장 스트릭은 현재 스트릭보다 작을 수 없습니다');
        }
        const now = new Date();
...
//# sourceMappingURL=StudyStreak.js.map