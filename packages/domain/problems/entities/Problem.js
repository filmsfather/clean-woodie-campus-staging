...
import { ProblemCreatedEvent, ProblemContentUpdatedEvent, ProblemAnswerUpdatedEvent, ProblemDifficultyChangedEvent, ProblemActivatedEvent, ProblemDeactivatedEvent } from '../events';
export class Problem extends AggregateRoot {
    ...
    updateContent(newContent) {
        ...
        // 도메인 이벤트 발행
        this.addDomainEvent(new ProblemContentUpdatedEvent(this.id, this.props.teacherId, previousContent, {
    ...
    updateCorrectAnswer(newAnswer) {
        ...
        this.props.updatedAt = new Date();
        // 도메인 이벤트 발행
        this.addDomainEvent(new ProblemAnswerUpdatedEvent(this.id, this.props.teacherId, previousAnswerType, newAnswer.type));
        return Result.ok();
    }
    // 난이도 변경
    changeDifficulty(newDifficulty) {
        ...
        this.props.updatedAt = new Date();
        // 도메인 이벤트 발행
        this.addDomainEvent(new ProblemDifficultyChangedEvent(this.id, this.props.teacherId, previousDifficulty, newDifficulty.level));
        return Result.ok();
    }
    // 태그 관리
    addTag(tag) {
    ...
    activate() {
        ...
        this.props.updatedAt = new Date();
        // 도메인 이벤트 발행
        this.addDomainEvent(new ProblemActivatedEvent(this.id, this.props.teacherId, this.props.content.title));
        return Result.ok();
    }
    deactivate() {
        if (!this.props.isActive) {
        ...
        this.props.updatedAt = new Date();
        // 도메인 이벤트 발행
        this.addDomainEvent(new ProblemDeactivatedEvent(this.id, this.props.teacherId, this.props.content.title));
        return Result.ok();
    }
    // 문제 소유권 확인
    isOwnedBy(teacherId) {
    ...
    static create(props, id) {
        ...
        // 도메인 이벤트 발행
        problem.addDomainEvent(new ProblemCreatedEvent(problem.id, props.teacherId, props.content.title, props.content.type.value, props.difficulty.level, uniqueTags.map(tag => tag.name)));
    ...
    static reconstitute = Problem.restore;
    // 직렬화 (영속성을 위한)
    toPersistence() {
        return {
            id: this.id.toString(),
