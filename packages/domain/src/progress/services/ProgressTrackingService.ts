...
export class ProgressTrackingService {
   ...
   * @returns 업데이트 결과
   */
  async updateProgressFromStudyRecord(
    ...
    try {
      // 1. 스트릭 업데이트
      const streakUpdateResult = await this.updateStudyStreak(
        studyRecord.studentId,
        studyRecord.createdAt
      )
      if (streakUpdateResult.isFailure) {
        return Result.fail<void>(`스트릭 업데이트 실패: ${streakUpdateResult.error}`)
      }

      // 2. 통계 업데이트
      const statisticsUpdateResult = await this.updateStatistics(
        studyRecord.studentId,
        problemSetId,
        studyRecord.isCorrect,
        studyRecord.responseTime || 0,
        totalProblemsInSet
      )
      if (statisticsUpdateResult.isFailure) {
        return Result.fail<void>(`통계 업데이트 실패: ${statisticsUpdateResult.error}`)
      }

      return Result.ok<void>()
    } catch (error) {
      return Result.fail<void>(`진도 추적 업데이트 중 오류 발생: ${error}`)
    }
  }
  ...
  private async updateStudyStreak(
    ...
    if (streakResult.isFailure) {
      return Result.fail<void>(streakResult.error)
    }
    ...
    if (!studyStreak) {
      ...
      if (newStreakResult.isFailure) {
        return Result.fail<void>(newStreakResult.error)
      }
  ...
  private async updateStatistics(
    ...
    // 기존 통계 조회
    const statisticsResult = await this.statisticsRepository.findByStudentAndProblemSet(
      studentId,
      problemSetId
    )
    if (statisticsResult.isFailure) {
      return Result.fail<void>(statisticsResult.error)
    }
    ...
    if (!statistics) {
      ...
      if (newStatisticsResult.isFailure) {
        return Result.fail<void>(newStatisticsResult.error)
      }
   ...
   * @returns 진도 현황 정보
   */
  async getStudentProgress(studentId: UniqueEntityID): Promise<Result<{
   ...
   * @returns 전체 지표
   */
  private calculateOverallMetrics(statistics: Statistics[]): {
    totalProblemSets: number
    completedProblemSets: number
    averageCompletionRate: number
    averageAccuracyRate: number
   ...
   * @returns 클래스 진도 현황
   */
  async getClassProgress(classId: string, problemSetId?: UniqueEntityID): Promise<Result<{
    streaks: StudyStreak[]
    statistics: Statistics[]
    classMetrics: {
   ...
   */
  private calculateClassMetrics(streaks: StudyStreak[], statistics: Statistics[]): {
    totalStudents: number
    activeStreakCount: number
    averageCurrentStreak: number
    averageCompletionRate: number
