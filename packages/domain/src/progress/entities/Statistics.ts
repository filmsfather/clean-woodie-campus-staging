...
interface StatisticsProps {
  ...
  averageResponseTime: number // 밀리초 단위
  createdAt: Date
  updatedAt: Date
}
...
export class Statistics extends AggregateRoot<UniqueEntityID> {
  ...
  public getCompletionRate(): number {
    if (this.props.totalProblems === 0) return 0
  ...
  public getAccuracyRate(): number {
    if (this.props.completedProblems === 0) return 0
   ...
   * @returns 정답 수 / 총 문제 수
   */
  public getOverallAccuracyRate(): number {
    if (this.props.totalProblems === 0) return 0
   ...
   */
  public getProgressStatus(): 'not_started' | 'in_progress' | 'completed' {
    if (this.props.completedProblems === 0) return 'not_started'
    if (this.props.completedProblems >= this.props.totalProblems) return 'completed'
    return 'in_progress'
  }
  ...
  public getPerformanceGrade(): 'A' | 'B' | 'C' | 'D' | 'F' {
    ...
    if (accuracyRate >= 0.9) return 'A'
    if (accuracyRate >= 0.8) return 'B'
    if (accuracyRate >= 0.7) return 'C'
    if (accuracyRate >= 0.6) return 'D'
    return 'F'
  }
   ...
   */
  public getAverageResponseTimeInSeconds(): number {
   ...
   */
  public getTotalTimeInMinutes(): number {
   ...
   */
  public recordStudyResult(isCorrect: boolean, responseTime: number): void {
    // 이전 상태 저장 (이벤트 발행용)
    const previousStats = {
    ...
    // 통계 업데이트 이벤트 발행
    this.addDomainEvent(new StatisticsUpdatedEvent(
      this.props.studentId,
      this.props.problemSetId,
      previousStats,
      currentStats,
      this.isCompleted(),
   ...
   */
  public updateStatistics(updateProps: StatisticsUpdateProps): Result<void> {
    // 유효성 검증
    if (updateProps.completedProblems !== undefined && updateProps.totalProblems !== undefined) {
      if (updateProps.completedProblems > updateProps.totalProblems) {
        return Result.fail<void>('완료한 문제 수는 총 문제 수를 초과할 수 없습니다')
      }
    }
    ...
    if (updateProps.correctAnswers !== undefined && updateProps.completedProblems !== undefined) {
      if (updateProps.correctAnswers > updateProps.completedProblems) {
        return Result.fail<void>('정답 수는 완료한 문제 수를 초과할 수 없습니다')
      }
    }
  ...
  public static create(props: {
    ...
    if (guardResult.isFailure) {
      return Result.fail<Statistics>(guardResult.error)
    }

    // 비즈니스 규칙 검증
    if (props.totalProblems < 0) {
      return Result.fail<Statistics>('총 문제 수는 음수가 될 수 없습니다')
    }
    ...
    if (completedProblems < 0) {
      return Result.fail<Statistics>('완료한 문제 수는 음수가 될 수 없습니다')
    }

    if (correctAnswers < 0) {
      return Result.fail<Statistics>('정답 수는 음수가 될 수 없습니다')
    }

    if (completedProblems > props.totalProblems) {
      return Result.fail<Statistics>('완료한 문제 수는 총 문제 수를 초과할 수 없습니다')
    }

    if (correctAnswers > completedProblems) {
      return Result.fail<Statistics>('정답 수는 완료한 문제 수를 초과할 수 없습니다')
    }

    if (totalTimeSpent < 0) {
      return Result.fail<Statistics>('총 소요 시간은 음수가 될 수 없습니다')
    }

    if (averageResponseTime < 0) {
      return Result.fail<Statistics>('평균 응답 시간은 음수가 될 수 없습니다')
    }
