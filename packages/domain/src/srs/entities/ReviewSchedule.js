    ...
    scheduleNotificationEvents(nextReviewAt) {
        ...
        if (this.shouldScheduleExtraReminders()) {
            ...
            if (earlyReminderTime.getTime() > Date.now()) {
                this.addDomainEvent(new ReviewNotificationScheduledEvent(this.id, this.studentId, this.problemId, NotificationType.reviewDue(), earlyReminderTime, nextReviewAt, 'high', // 어려운 문제라서 높은 우선순위
                {
