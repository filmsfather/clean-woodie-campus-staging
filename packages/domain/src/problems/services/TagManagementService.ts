...
import { 
  ...
  TagManagementConstants,
  ProblemManagementProfiles 
} from './ProblemManagementConfig';
...
export class TagManagementService {
  ...
  // 태그 사용 빈도 분석 (옵션화)
  public static analyzeTagUsage(
    problemTags: Tag[][],
    options: TagManagementOptions = {}
  ): Array<{ tag: Tag; count: number; percentage: number }> {
    const config = { 
      ...ProblemManagementProfiles.DEFAULT_CONFIG.tags, 
      ...options 
    };
  ...
  // 태그 계층구조 분석 (옵션화)
  public static analyzeTagHierarchy(
    tags: Tag[],
    options: TagManagementOptions = {}
  ): Map<string, Tag[]> {
    const config = { 
    ...
    const hierarchy = new Map<string, Tag[]>();
  ...
  // 태그 클러스터링 (옵션화)
  public static clusterSimilarTags(
    tags: Tag[],
    options: TagManagementOptions = {}
  ): Tag[][] {
    const config = { 
  ...
  public static recommendTags(
    ...
    const recommendations: Array<{ tag: Tag; relevance: number }> = [];
  ...
  public static normalizeTagSet(
    ...
    if (uniqueTags.length > maxTags) {
      return Result.fail<Tag[]>(`Cannot have more than ${maxTags} tags`);
    }

    // 빈 태그 제거
    const validTags = uniqueTags.filter(tag => 
      tag.name && tag.name.trim().length > 0
    );

    return Result.ok<Tag[]>(validTags);
  }
  ...
  // 태그 통계 정보
  public static getTagStatistics(problemTags: Tag[][]): {
...
export class ConfigurableTagManagementService {
  private options: TagManagementOptions;
  ...
  analyzeTagUsage(problemTags: Tag[][]): Array<{ tag: Tag; count: number; percentage: number }> {
