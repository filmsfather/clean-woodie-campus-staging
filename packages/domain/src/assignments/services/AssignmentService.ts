...
export interface CreateAssignmentRequest {
  ...
  title: string;
  description?: string;
  dueDate: Date; // 원본 Date는 유지하되 내부에서 DueDate로 변환
  timezone?: string;
  maxAttempts?: number;
  classIds?: string[];
  studentIds?: string[];
...
export class AssignmentService {
  ...
  async createAssignmentWithTargets(
    ...
    if (dueDateResult.isFailure) {
      return Result.fail<Assignment>(`Invalid due date: ${dueDateResult.error}`);
    }
    ...
    if (assignmentResult.isFailure) {
      return Result.fail<Assignment>(assignmentResult.error);
    }
    ...
    if (request.classIds?.length || request.studentIds?.length) {
      const assignResult = await this.assignTargets(
        assignment,
        {
      ...
      if (assignResult.isFailure) {
        return Result.fail<Assignment>(assignResult.error);
      }
    }
    ...
    if (saveResult.isFailure) {
      return Result.fail<Assignment>(`Failed to save assignment: ${saveResult.error}`);
    }
  ...
  async assignTargets(
    ...
    const targetIdentifiers: AssignmentTargetIdentifier[] = [];
    ...
    if (targets.classIds) {
      for (const classIdStr of targets.classIds) {
        ...
        if (classIdResult.isFailure) {
          return Result.fail<void>(`Invalid class ID '${classIdStr}': ${classIdResult.error}`);
        }

        const identifierResult = AssignmentTargetIdentifier.createForClass(classIdResult.value);
        if (identifierResult.isFailure) {
          return Result.fail<void>(identifierResult.error);
        }
    ...
    if (targets.studentIds) {
      for (const studentIdStr of targets.studentIds) {
        ...
        if (studentIdResult.isFailure) {
          return Result.fail<void>(`Invalid student ID '${studentIdStr}': ${studentIdResult.error}`);
        }
    ...
    if (targetIdentifiers.length > 0) {
      ...
      if (assignResult.isFailure) {
        return Result.fail<void>(assignResult.error);
      }
    }
  ...
  async revokeAssignmentTargets(
    ...
    if (assignmentResult.isFailure) {
      return Result.fail<void>(`Assignment not found: ${assignmentResult.error}`);
    }
    ...
    if (!assignment.isOwnedBy(revokedBy)) {
      return Result.fail<void>('Only the assignment owner can revoke assignments');
    }
    ...
    const saveResult = await this.assignmentRepository.save(assignment);
    if (saveResult.isFailure) {
      return Result.fail<void>(`Failed to save assignment: ${saveResult.error}`);
    }
  ...
  async activateAssignment(
    ...
    if (!assignment.isOwnedBy(teacherId)) {
      return Result.fail<void>('Only the assignment owner can activate assignments');
    }

    // 배정이 있는지 확인
    if (!assignment.hasActiveAssignments()) {
      return Result.fail<void>('Cannot activate assignment without any assigned targets');
    }

    // 활성화
    const activateResult = assignment.activate();
    if (activateResult.isFailure) {
      return Result.fail<void>(activateResult.error);
    }
  ...
  // 학생이 접근 가능한 과제 목록 조회
  async getAccessibleAssignmentsForStudent(studentId: string): Promise<Result<Assignment[]>> {
    const studentIdResult = StudentId.create(studentId);
    if (studentIdResult.isFailure) {
      return Result.fail<Assignment[]>(studentIdResult.error);
    }
    ...
    return Result.ok<Assignment[]>(accessibleAssignments);
  }

  // 반에 배정된 과제 목록 조회
  async getAssignmentsForClass(classId: string): Promise<Result<Assignment[]>> {
    const classIdResult = ClassId.create(classId);
    if (classIdResult.isFailure) {
      return Result.fail<Assignment[]>(classIdResult.error);
    }
    ...
    return Result.ok<Assignment[]>(classAssignments);
  }

  // 마감일 관리 메서드들

  // 과제 마감일 연장
  async extendAssignmentDueDate(
    assignmentId: UniqueEntityID,
    ...
    // 권한 검증
    if (!assignment.isOwnedBy(teacherId)) {
      return Result.fail<void>('Only the assignment owner can extend due dates');
    }

    // 마감일 연장
    const extendResult = assignment.extendDueDate(additionalHours, teacherId);
    if (extendResult.isFailure) {
      return Result.fail<void>(extendResult.error);
    }
  ...
  async changeAssignmentDueDate(
    ...
    // 권한 검증
    if (!assignment.isOwnedBy(teacherId)) {
      return Result.fail<void>('Only the assignment owner can change due dates');
    }

    // 마감일 변경
    const changeResult = assignment.changeDueDateTo(newDueDate, timezone, teacherId);
    if (changeResult.isFailure) {
      return Result.fail<void>(changeResult.error);
    }
  ...
  async getAssignmentsDueSoon(
    ...
    const dueSoonAssignments = assignmentsResult.value.filter(assignment => 
      assignment.isDueSoon(hoursThreshold)
    );

    return Result.ok<Assignment[]>(dueSoonAssignments);
  }

  // 마감된 과제 목록 조회
  async getOverdueAssignments(): Promise<Result<Assignment[]>> {
    const assignmentsResult = await this.assignmentRepository.findActiveAssignments();
    if (assignmentsResult.isFailure) {
      return Result.fail<Assignment[]>(assignmentsResult.error);
    }

    const overdueAssignments = assignmentsResult.value.filter(assignment => 
      assignment.isOverdue()
    );

    return Result.ok<Assignment[]>(overdueAssignments);
  }

  // 교사의 과제별 마감일 현황 조회
  async getTeacherAssignmentsDueDateStatus(
    teacherId: string
  ): Promise<Result<AssignmentDueDateStatus[]>> {
    const assignmentsResult = await this.assignmentRepository.findByTeacherId(teacherId);
    if (assignmentsResult.isFailure) {
      return Result.fail<AssignmentDueDateStatus[]>(assignmentsResult.error);
    }
    ...
    return Result.ok<AssignmentDueDateStatus[]>(statusList);
  }

  // 마감일 기반 자동 과제 마감 처리
  async processOverdueAssignments(): Promise<Result<number>> {
    const overdueAssignmentsResult = await this.getOverdueAssignments();
    if (overdueAssignmentsResult.isFailure) {
      return Result.fail<number>(overdueAssignmentsResult.error);
    }
    ...
    return Result.ok<number>(closedCount);
  }
  ...
  private async validateAssignmentTargets(
    ...
    if (classTargets.length > 0 && studentTargets.length > 0) {
      ...
      return Result.fail<void>(
        'Cannot assign to both class and individual students in the same operation. ' +
