...
export class UserAchievement extends AggregateRoot<UserAchievementProps> {
  ...
  public static create(props: UserAchievementProps, id?: UniqueEntityID): Result<UserAchievement> {
    // 미래 날짜 검증
  ...
  public static createAndNotify(
    ...
    // 업적 획득 이벤트 발생
    const event = new AchievementEarnedEvent(
      studentId,
      achievementId,
      userAchievement.id,
      props.earnedAt
   ...
   */
  public getDaysEarned(currentDate: Date = new Date()): number {
   ...
   */
  public isRecentlyEarned(currentDate: Date = new Date()): boolean {
