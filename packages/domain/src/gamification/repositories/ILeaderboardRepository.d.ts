...
export interface ILeaderboardRepository {
     ...
     * 리더보드 조회
     */
    getLeaderboard(type: LeaderboardType, limit: number, periodStart?: Date, periodEnd?: Date): Promise<Result<LeaderboardEntry[]>>;
    /**
     * 학생의 특정 리더보드에서의 순위 조회
     */
    getStudentRank(studentId: StudentId, type: LeaderboardType, periodStart?: Date, periodEnd?: Date): Promise<Result<LeaderboardEntry | null>>;
    /**
     * 리더보드 엔트리 저장
     */
     ...
     * 특정 기간의 리더보드 엔트리들 삭제
     */
    deleteByPeriod(type: LeaderboardType, periodStart: Date, periodEnd: Date): Promise<Result<void>>;
    /**
     ...
     */
    getTokenEarnedByPeriodData(startDate: Date, endDate: Date, limit: number): Promise<Result<{
        studentId: StudentId;
        score: number;
    }[]>>;
}
