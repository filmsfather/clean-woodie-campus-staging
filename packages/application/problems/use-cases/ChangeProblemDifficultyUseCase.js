import { Result, Difficulty, UniqueEntityID } from '@woodie/domain';
export class ChangeProblemDifficultyUseCase {
    problemRepository;
    ...
    async execute(request) {
        try {
            ...
            if (typeof request.difficultyLevel !== 'number') {
                return Result.fail('Difficulty level must be a number');
            }
            // 2. 문제 조회
            ...
            if (!problem.isOwnedBy(request.teacherId)) {
                return Result.fail('Access denied: Only problem owner can change difficulty');
            }
            // 4. 새로운 난이도 생성
            const newDifficultyResult = Difficulty.create(request.difficultyLevel);
            if (newDifficultyResult.isFailure) {
                return Result.fail(`Invalid difficulty: ${newDifficultyResult.errorValue}`);
            }
            // 5. 도메인 로직 실행
            const updateResult = problem.changeDifficulty(newDifficultyResult.value);
            if (updateResult.isFailure) {
                return Result.fail(`Failed to change difficulty: ${updateResult.errorValue}`);
            }
            // 6. 저장
...
//# sourceMappingURL=ChangeProblemDifficultyUseCase.js.map