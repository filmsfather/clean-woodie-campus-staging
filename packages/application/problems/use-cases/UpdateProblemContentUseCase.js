import { Result, ProblemContent, UniqueEntityID } from '@woodie/domain';
export class UpdateProblemContentUseCase {
    problemRepository;
    ...
    async execute(request) {
        try {
            ...
            if (!problem.isOwnedBy(request.teacherId)) {
                return Result.fail('Access denied: Only problem owner can update content');
            }
            // 4. 새로운 문제 내용 생성
            ...
            if (newContentResult.isFailure) {
                return Result.fail(`Invalid content: ${newContentResult.errorValue}`);
            }
            // 5. 도메인 로직 실행
            const updateResult = problem.updateContent(newContentResult.value);
            if (updateResult.isFailure) {
                return Result.fail(`Failed to update content: ${updateResult.errorValue}`);
            }
            // 6. 저장
...
//# sourceMappingURL=UpdateProblemContentUseCase.js.map