...
export class ProblemSearchService {
    ...
    logger;
    cacheService;
    constructor(problemRepository, logger, cacheService) {
        this.problemRepository = problemRepository;
        this.logger = logger;
        this.cacheService = cacheService;
    }
    async searchProblems(criteria) {
        ...
        try {
            ...
            // Repository 호출
            const searchResult = await this.problemRepository.searchProblems(repositoryFilter, repositoryPagination, repositorySort);
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.SEARCH_FAILED, 'Unexpected error during problem search', { criteria, correlationId }, error);
    ...
    async findProblemById(problemId, requesterId) {
        const correlationId = this.generateCorrelationId();
        try {
            ...
            // Repository 호출
            const problemResult = await this.problemRepository.findById(new UniqueEntityID(problemId));
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.PROBLEM_NOT_FOUND, 'Unexpected error finding problem', { problemId, requesterId, correlationId }, error);
    ...
    async findProblemsByTeacher(teacherId, includeInactive) {
        const correlationId = this.generateCorrelationId();
        try {
            ...
            const searchResult = await this.problemRepository.searchProblems(repositoryFilter, undefined, undefined);
            if (searchResult.isFailure) {
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.SEARCH_FAILED, 'Failed to find problems by teacher', { teacherId, includeInactive, correlationId }, error);
    ...
    async findProblemsByTags(tags, teacherId) {
        const correlationId = this.generateCorrelationId();
        try {
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.SEARCH_FAILED, 'Failed to find problems by tags', { tags, teacherId, correlationId }, error);
    ...
    async findPopularProblems(limit = 10, teacherId) {
        const correlationId = this.generateCorrelationId();
        try {
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.SEARCH_FAILED, 'Failed to find popular problems', { limit, teacherId, correlationId }, error);
    ...
    async findSimilarProblems(problemId, teacherId, limit = 5) {
        const correlationId = this.generateCorrelationId();
        try {
            ...
            // 유사 문제 검색
            const similarResult = await this.problemRepository.findSimilarProblems(problem, teacherId, limit);
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.SEARCH_FAILED, 'Failed to find similar problems', { problemId, teacherId, limit, correlationId }, error);
    ...
    validateSearchCriteria(criteria) {
        ...
        if (criteria.limit && (criteria.limit <= 0 || criteria.limit > 100)) {
            const error = new ProblemBankError(ProblemBankErrorCode.PAGINATION_ERROR, 'Limit must be between 1 and 100');
            return Result.fail(error.message);
        }
        if (criteria.offset && criteria.offset < 0) {
            const error = new ProblemBankError(ProblemBankErrorCode.PAGINATION_ERROR, 'Offset must be greater than or equal to 0');
            return Result.fail(error.message);
        }
        // 난이도 검증
        if (criteria.difficultyLevel && (criteria.difficultyLevel < 1 || criteria.difficultyLevel > 5)) {
            const error = new ProblemBankError(ProblemBankErrorCode.INVALID_SEARCH_FILTER, 'Difficulty level must be between 1 and 5');
            return Result.fail(error.message);
        }
        if (criteria.difficultyRange) {
            const { min, max } = criteria.difficultyRange;
            if (min < 1 || max > 5 || min > max) {
                const error = new ProblemBankError(ProblemBankErrorCode.INVALID_SEARCH_FILTER, 'Invalid difficulty range');
                return Result.fail(error.message);
            }
        }
        return Result.ok();
    }
    validateSearchRequest(filter, pagination, sort) {
        // 페이지네이션 검증
        if (pagination) {
            ...
            if (pagination.strategy === 'offset' && pagination.page !== undefined && pagination.page < 1) {
                const error = new ProblemBankError(ProblemBankErrorCode.PAGINATION_ERROR, 'Page number must be greater than 0');
                return Result.fail(error.message);
            }
        }
        // 정렬 검증
        if (sort) {
            ...
            if (!validFields.includes(sort.field)) {
                const error = new ProblemBankError(ProblemBankErrorCode.SORT_PARAMETER_INVALID, `Invalid sort field: ${sort.field}`);
                return Result.fail(error.message);
            }
            if (!validDirections.includes(sort.direction)) {
                const error = new ProblemBankError(ProblemBankErrorCode.SORT_PARAMETER_INVALID, `Invalid sort direction: ${sort.direction}`);
                return Result.fail(error.message);
            }
        }
        // 필터 검증
        if (filter.difficulties) {
            ...
            if (invalidDifficulties.length > 0) {
                const error = new ProblemBankError(ProblemBankErrorCode.INVALID_SEARCH_FILTER, `Invalid difficulty levels: ${invalidDifficulties.join(', ')}`);
                return Result.fail(error.message);
            }
        }
        return Result.ok();
    }
    mapCriteriaToRepositoryFilter(criteria) {
        return {
            teacherId: criteria.teacherId,
            difficultyLevels: criteria.difficultyLevel ? [criteria.difficultyLevel] :
                criteria.difficultyRange ? Array.from({ length: criteria.difficultyRange.max - criteria.difficultyRange.min + 1 }, (_, i) => criteria.difficultyRange.min + i) : undefined,
    ...
    mapCriteriaToPagination(criteria) {
    ...
    mapCriteriaToSort(criteria) {
        return {
            field: 'createdAt',
            direction: 'DESC'
        };
    }
    mapToRepositoryFilter(filter) {
        return {
    ...
    mapToRepositoryPagination(pagination) {
        if (!pagination)
            return undefined;
        return {
            ...
            cursor: pagination.cursor ? {
                ...
                value: pagination.cursor.field.includes('At')
                    ? new Date(pagination.cursor.value)
                    : pagination.cursor.value,
                direction: pagination.cursor.direction
            } : undefined
        };
    }
    mapToRepositorySort(sort) {
        if (!sort)
            return undefined;
        return {
            field: sort.field,
    ...
    mapToSearchResponseDto(searchResult) {
        return {
            problems: searchResult.problems.map((p) => this.mapProblemToDto(p)),
    ...
    async cacheSearchResult(cacheKey, result) {
        if (!this.cacheService)
            return;
        try {
