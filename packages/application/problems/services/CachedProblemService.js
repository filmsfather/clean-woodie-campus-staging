 ...
 */
export class CachedProblemService {
    problemRepository;
    cacheService;
    constructor(problemRepository, cacheService) {
    ...
    async getProblemById(id) {
        ...
        try {
            // 1. 캐시에서 먼저 조회
            const cachedProblem = await this.cacheService.get(cacheKey);
            if (cachedProblem) {
     ...
     * 교사별 문제 조회 (캐싱 적용)
     */
    async getProblemsByTeacher(teacherId, filters) {
        const cacheKey = CacheKeys.TEACHER_PROBLEMS(teacherId.toString(), this.serializeFilters(filters));
        try {
            // 1. 캐시에서 먼저 조회
            const cachedProblems = await this.cacheService.get(cacheKey);
            if (cachedProblems) {
                const problems = [];
     ...
     * 태그별 문제 조회 (캐싱 적용)
     */
    async getProblemsByTags(tags, filters) {
     ...
     * 인기 문제 조회 (캐싱 적용)
     */
    async getPopularProblems(limit = 20) {
     ...
     * 교사 관련 문제 캐시 무효화
     */
    async invalidateTeacherProblemCaches(teacherId) {
     ...
     * 태그 관련 캐시 무효화
     */
    async invalidateTagCaches(tags) {
     ...
     * 문제 관련 캐시 무효화
     */
    async invalidateProblemCaches(problemId, originalProblem, updates) {
...
//# sourceMappingURL=CachedProblemService.js.map