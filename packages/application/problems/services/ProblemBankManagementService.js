...
import { CacheTags } from '../../common/interfaces/ICacheService';
...
export class ProblemBankManagementService {
    ...
    async cloneProblems(request) {
        ...
        try {
            ...
            // 소유권 일괄 확인 (N+1 방지)
            const ownershipResult = await this.bulkVerifyOwnership(request.problemIds, request.targetTeacherId // 실제로는 현재 요청자의 ID를 받아야 함
            );
            ...
            // 권한이 없는 문제들 필터링
            const authorizedProblemIds = request.problemIds.filter(id => ownershipMap.get(id) === true);
            const unauthorizedProblemIds = request.problemIds.filter(id => ownershipMap.get(id) !== true);
            ...
            if (authorizedProblemIds.length === 0) {
                const error = new ProblemBankError(ProblemBankErrorCode.CLONE_PERMISSION_DENIED, 'No problems are authorized for cloning', { request, correlationId });
            ...
            const cloneResult = await this.problemRepository.cloneProblems(problemEntityIds, request.targetTeacherId, cloneOptions);
            if (cloneResult.isFailure) {
                const error = new ProblemBankError(ProblemBankErrorCode.CLONE_OPERATION_FAILED, 'Bulk clone operation failed', { request, correlationId }, new Error(cloneResult.error));
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.CLONE_OPERATION_FAILED, 'Unexpected error during problem cloning', { request, correlationId }, error);
    ...
    async bulkUpdateActiveStatus(request) {
        const startTime = Date.now();
        const correlationId = this.generateCorrelationId();
        try {
            ...
            const updateResult = await this.problemRepository.bulkUpdateActiveStatus(problemEntityIds, request.isActive, request.teacherId);
            if (updateResult.isFailure) {
                const error = ProblemBankErrorFactory.bulkOperationFailed('status update', 0, request.problemIds.length, [updateResult.error]);
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.BULK_OPERATION_FAILED, 'Unexpected error during bulk status update', { request, correlationId }, error);
    ...
    async bulkUpdateTags(request) {
        ...
        try {
            ...
            if (request.tags.length === 0) {
                const error = new ProblemBankError(ProblemBankErrorCode.BULK_VALIDATION_FAILED, 'Tags array cannot be empty', { request });
                return Result.fail(error.message);
            }
            // Repository 호출
            const problemEntityIds = request.problemIds.map(id => new UniqueEntityID(id));
            const updateResult = await this.problemRepository.bulkUpdateTags(problemEntityIds, request.tags, request.teacherId, request.operation);
            if (updateResult.isFailure) {
                const error = ProblemBankErrorFactory.bulkOperationFailed('tags update', 0, request.problemIds.length, [updateResult.error]);
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.BULK_OPERATION_FAILED, 'Unexpected error during bulk tags update', { request, correlationId }, error);
    ...
    async checkBulkPermissions(problemIds, teacherId) {
        const startTime = Date.now();
        const correlationId = this.generateCorrelationId();
        try {
            ...
            if (ownershipResult.isFailure || accessResult.isFailure) {
                const error = new ProblemBankError(ProblemBankErrorCode.OWNERSHIP_VERIFICATION_FAILED, 'Failed to verify bulk permissions', { problemIds, teacherId, correlationId });
                return Result.fail(error.message);
            }
            const ownershipMap = new Map(ownershipResult.value.map(item => [item.id, item.isOwner]));
            const accessMap = new Map(accessResult.value.map(item => [item.id, item.canAccess]));
            // 권한 정보 구성
            const permissions = problemIds.map(problemId => {
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.OWNERSHIP_VERIFICATION_FAILED, 'Unexpected error during bulk permission check', { problemIds, teacherId, correlationId }, error);
    ...
    async validateBulkOperation(request) {
        const correlationId = this.generateCorrelationId();
        try {
            const errors = [];
            const warnings = [];
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.BULK_VALIDATION_FAILED, 'Failed to validate bulk operation', { request, correlationId }, error);
            return Result.fail(problemBankError.message);
        }
    }
    // === Private 헬퍼 메서드들 ===
    async validateCloneRequest(request) {
        if (!request.targetTeacherId || request.targetTeacherId.trim().length === 0) {
            const error = new ProblemBankError(ProblemBankErrorCode.CLONE_VALIDATION_FAILED, 'Target teacher ID is required');
            return Result.fail(error.message);
        }
        if (!request.problemIds || request.problemIds.length === 0) {
            const error = new ProblemBankError(ProblemBankErrorCode.CLONE_VALIDATION_FAILED, 'Problem IDs array cannot be empty');
            return Result.fail(error.message);
        }
        if (request.problemIds.length > 50) {
            const error = new ProblemBankError(ProblemBankErrorCode.CLONE_VALIDATION_FAILED, 'Cannot clone more than 50 problems at once');
            return Result.fail(error.message);
        }
        return Result.ok();
    }
    ...
    async invalidateTeacherCaches(teacherIds) {
        if (!this.cacheService)
            return;
        try {
...
//# sourceMappingURL=ProblemBankManagementService.js.map