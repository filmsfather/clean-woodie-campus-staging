...
export interface GetClassProgressRequest {
  classId: string
  problemSetId?: string // 특정 문제집에 대한 진도만 조회 (선택적)
  teacherId?: string // 요청 교사 ID (권한 확인용)
 ...
 * - 성과 분석과 인사이트를 함께 제공
 */
export class GetClassProgressUseCase extends BaseUseCase<GetClassProgressRequest, GetClassProgressResponse> {
  constructor(
  ...
  async execute(request: GetClassProgressRequest): Promise<Result<GetClassProgressResponse>> {
    try {
      // 1. 입력 유효성 검증
      const validationResult = this.validateRequest(request)
      if (validationResult.isFailure) {
        return Result.fail<GetClassProgressResponse>(validationResult.error)
      }

      // 2. 권한 확인 (실제 구현에서는 더 엄격한 검증 필요)
      const authResult = this.checkAuthorization(request)
      if (authResult.isFailure) {
        return Result.fail<GetClassProgressResponse>(authResult.error)
      }

      // 3. 클래스 진도 조회
      const progressResult = await this.progressService.getClassProgress(
        request.classId, 
        request.problemSetId
      )
      if (progressResult.isFailure) {
        return Result.fail<GetClassProgressResponse>(progressResult.error)
      }
      ...
      return Result.ok<GetClassProgressResponse>(response)
    } catch (error) {
      return Result.fail<GetClassProgressResponse>(`Failed to get class progress: ${error}`)
    }
  }

  private validateRequest(request: GetClassProgressRequest): Result<void> {
    if (!request.classId || request.classId.trim() === '') {
      return Result.fail<void>('Class ID is required')
    }

    const validSortOptions = ['name', 'streak', 'completion', 'accuracy']
    if (request.sortBy && !validSortOptions.includes(request.sortBy)) {
      return Result.fail<void>(`Invalid sort option. Must be one of: ${validSortOptions.join(', ')}`)
    }
  ...
  private checkAuthorization(request: GetClassProgressRequest): Result<void> {
    // TODO: 실제 권한 확인 로직 구현
    // - 교사는 담당 클래스만 접근 가능
    // - 관리자는 모든 클래스 접근 가능
    return Result.ok<void>()
  ...
  private generateInsights(classProgress: ClassProgressDto) {
    const insights = []
    ...
    // 2. 도움이 필요한 학생 식별
    const strugglingStudents = statistics.filter(s => 
      s.completionRate < 0.3 || s.accuracyRate < 0.5
    )
    ...
    if (classMetrics.averageCompletionRate < 0.5) {
      insights.push({
        type: 'improvement_needed' as const,
  ...
  private generateSummary(classProgress: ClassProgressDto, insights: any[]) {
