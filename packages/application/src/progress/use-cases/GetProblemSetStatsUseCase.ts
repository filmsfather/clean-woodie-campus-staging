...
export interface GetProblemSetStatsRequest {
  ...
  includeIndividualStats?: boolean // 개별 학생 통계 포함 여부
 ...
 * - 통계 분석 및 개선 인사이트 제공
 */
export class GetProblemSetStatsUseCase extends BaseUseCase<GetProblemSetStatsRequest, GetProblemSetStatsResponse> {
  constructor(
  ...
  async execute(request: GetProblemSetStatsRequest): Promise<Result<GetProblemSetStatsResponse>> {
    try {
      // 1. 입력 유효성 검증
      const validationResult = this.validateRequest(request)
      if (validationResult.isFailure) {
        return Result.fail<GetProblemSetStatsResponse>(validationResult.error)
      }

      // 2. 권한 확인
      const authResult = this.checkAuthorization(request)
      if (authResult.isFailure) {
        return Result.fail<GetProblemSetStatsResponse>(authResult.error)
      }

      // 3. 문제집 통계 조회
      const summaryResult = await this.progressService.getProblemSetStatsSummary(request.problemSetId)
      if (summaryResult.isFailure) {
        return Result.fail<GetProblemSetStatsResponse>(summaryResult.error)
      }
      ...
      // 7. 응답 구성
      const response: GetProblemSetStatsResponse = {
        summary,
        insights,
        comparisons
      }

      return Result.ok<GetProblemSetStatsResponse>(response)
    } catch (error) {
      return Result.fail<GetProblemSetStatsResponse>(`Failed to get problem set stats: ${error}`)
    }
  }

  private validateRequest(request: GetProblemSetStatsRequest): Result<void> {
  ...
  private checkAuthorization(request: GetProblemSetStatsRequest): Result<void> {
    // 관리자는 모든 데이터에 접근 가능
    ...
    return Result.fail<void>('Access denied: Insufficient permissions to view problem set statistics')
  }

  private generateInsights(summary: ProblemSetStatsSummaryDto) {
    const insights = []
  ...
  private async generateComparisons(summary: ProblemSetStatsSummaryDto) {
