...
import { StudyRecord } from '@woodie/domain'
import { StudyStreak, Statistics, ProgressTrackingService } from '@woodie/domain'
...
export class ProgressTrackingApplicationService {
  constructor(
    ...
    private studyStreakRepository: IStudyStreakRepository,
   ...
   */
  async updateProgressFromStudyRecord(request: UpdateProgressRequest): Promise<Result<UpdateProgressResponse>> {
    try {
      ...
      const previousStatisticsResult = await this.statisticsRepository.findByStudentAndProblemSet(
        studentId, 
        problemSetId
      )
      ...
      const updateResult = await this.progressTrackingService.updateProgressFromStudyRecord(
        ...
        problemSetId,
        request.totalProblemsInSet
      )

      if (updateResult.isFailure) {
        return Result.fail<UpdateProgressResponse>(`Failed to update progress: ${updateResult.error}`)
      }

      // 업데이트 후 상태 조회
      const updatedStreakResult = await this.studyStreakRepository.findByStudentId(studentId)
      const updatedStatisticsResult = await this.statisticsRepository.findByStudentAndProblemSet(
        studentId, 
        problemSetId
      )
      ...
      return Result.ok<UpdateProgressResponse>(response)
    } catch (error) {
      return Result.fail<UpdateProgressResponse>(`Unexpected error updating progress: ${error}`)
    }
  }
   ...
   * 학생의 전체 진도 현황 조회
   */
  async getStudentProgress(studentId: string): Promise<Result<StudentProgressDto>> {
    try {
      ...
      if (progressResult.isFailure) {
        return Result.fail<StudentProgressDto>(progressResult.error)
      }

      const progress = progressResult.value
      
      // DTO 변환
      const dto: StudentProgressDto = {
        studentId,
      ...
      return Result.ok<StudentProgressDto>(dto)
    } catch (error) {
      return Result.fail<StudentProgressDto>(`Failed to get student progress: ${error}`)
    }
  }
   ...
   * 클래스 전체 진도 현황 조회
   */
  async getClassProgress(classId: string, problemSetId?: string): Promise<Result<ClassProgressDto>> {
    try {
      ...
      if (progressResult.isFailure) {
        return Result.fail<ClassProgressDto>(progressResult.error)
      }

      const progress = progressResult.value

      // DTO 변환
      const dto: ClassProgressDto = {
        classId,
      ...
      return Result.ok<ClassProgressDto>(dto)
    } catch (error) {
      return Result.fail<ClassProgressDto>(`Failed to get class progress: ${error}`)
    }
  }
   ...
   * 스트릭 순위 조회
   */
  async getStreakRankings(limit: number = 10, studentId?: string): Promise<Result<StreakRankingDto>> {
    try {
      const topStreaksResult = await this.studyStreakRepository.findTopStreaks(limit)
      if (topStreaksResult.isFailure) {
        return Result.fail<StreakRankingDto>(topStreaksResult.error)
      }
      ...
      const dto: StreakRankingDto = {
        rankings,
        myRanking
      }

      return Result.ok<StreakRankingDto>(dto)
    } catch (error) {
      return Result.fail<StreakRankingDto>(`Failed to get streak rankings: ${error}`)
    }
  }
   ...
   * 문제집별 통계 요약 조회
   */
  async getProblemSetStatsSummary(problemSetId: string): Promise<Result<ProblemSetStatsSummaryDto>> {
    try {
      ...
      if (statisticsResult.isFailure) {
        return Result.fail<ProblemSetStatsSummaryDto>(statisticsResult.error)
      }
      ...
      if (averageResult.isFailure) {
        return Result.fail<ProblemSetStatsSummaryDto>(averageResult.error)
      }
      ...
      const dto: ProblemSetStatsSummaryDto = {
        problemSetId,
      ...
      return Result.ok<ProblemSetStatsSummaryDto>(dto)
    } catch (error) {
      return Result.fail<ProblemSetStatsSummaryDto>(`Failed to get problem set stats summary: ${error}`)
    }
  }
   ...
   */
  private toStudyStreakDto(streak: StudyStreak): StudyStreakDto {
    return {
   ...
   */
  private toStatisticsDto(stats: Statistics): StatisticsDto {
    return {
  ...
  private buildStatisticsUpdateInfo(
    previousStats: Statistics | null,
    updatedStats: Statistics | null
  ) {
   ...
   * 학생의 전체 효율성 점수 계산
   */
  private async calculateEfficiencyScore(studentId: UniqueEntityID): Promise<number> {
    try {
