...
import { Result, UniqueEntityID, ProblemSet, ProblemSetTitle, ProblemSetDescription, IProblemSetRepository } from '@woodie/domain'
import { CreateProblemSetRequest, CreateProblemSetResponse, ProblemSetDto } from '../dto/ProblemSetDto'
 ...
 */
export class CreateProblemSetUseCase extends BaseUseCase<CreateProblemSetRequest, CreateProblemSetResponse> {
  constructor(
  ...
  async execute(request: CreateProblemSetRequest): Promise<Result<CreateProblemSetResponse>> {
    try {
      // 1. 입력 유효성 검증
      const validationResult = this.validateRequest(request)
      if (validationResult.isFailure) {
        return Result.fail<CreateProblemSetResponse>(validationResult.error)
      }
      ...
      if (titleExistsResult.isSuccess) {
        return Result.fail<CreateProblemSetResponse>(
      ...
      if (titleResult.isFailure) {
        return Result.fail<CreateProblemSetResponse>(`Invalid title: ${titleResult.error}`)
      }
      ...
      if (request.description) {
        ...
        if (descriptionResult.isFailure) {
          return Result.fail<CreateProblemSetResponse>(`Invalid description: ${descriptionResult.error}`)
        }
      ...
      if (problemSetResult.isFailure) {
        return Result.fail<CreateProblemSetResponse>(`Failed to create problem set: ${problemSetResult.error}`)
      }
      ...
      if (request.initialProblems && request.initialProblems.length > 0) {
        for (const problemData of request.initialProblems) {
          const addResult = problemSet.addProblem(
            new UniqueEntityID(problemData.problemId),
      ...
      if (saveResult.isFailure) {
        return Result.fail<CreateProblemSetResponse>(`Failed to save problem set: ${saveResult.error}`)
      }

      // 7. 응답 DTO 생성
      const problemSetDto = this.mapToDto(problemSet, request.isPublic, request.isShared)
      
      const response: CreateProblemSetResponse = {
        problemSet: problemSetDto,
        ...(warnings.length > 0 && { validationWarnings: warnings })
      }

      return Result.ok<CreateProblemSetResponse>(response)

    } catch (error) {
      return Result.fail<CreateProblemSetResponse>(`Unexpected error creating problem set: ${error}`)
    }
  }

  private validateRequest(request: CreateProblemSetRequest): Result<void> {
    const errors: string[] = []
  ...
  private mapToDto(problemSet: ProblemSet, isPublic?: boolean, isShared?: boolean): ProblemSetDto {
    return {
