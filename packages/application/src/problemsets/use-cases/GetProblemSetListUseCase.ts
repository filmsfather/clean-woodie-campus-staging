...
import { GetProblemSetListRequest, GetProblemSetListResponse, ProblemSetDto } from '../dto/ProblemSetDto'
 ...
 * - 공유 가능한 문제집들 우선 표시
 */
export class GetProblemSetListUseCase extends BaseUseCase<GetProblemSetListRequest, GetProblemSetListResponse> {
  constructor(
  ...
  async execute(request: GetProblemSetListRequest): Promise<Result<GetProblemSetListResponse>> {
    try {
      // 1. 입력 유효성 검증
      const validationResult = this.validateRequest(request)
      if (validationResult.isFailure) {
        return Result.fail<GetProblemSetListResponse>(validationResult.error)
      }

      // 2. 권한에 따른 조회 범위 결정
      const scopeResult = this.determineScopeByRole(request)
      if (scopeResult.isFailure) {
        return Result.fail<GetProblemSetListResponse>(scopeResult.error)
      }
      ...
      if (problemSetsResult.isFailure) {
        return Result.fail<GetProblemSetListResponse>(problemSetsResult.error)
      }
      ...
      // 6. 응답 구성
      const response: GetProblemSetListResponse = {
      ...
      return Result.ok<GetProblemSetListResponse>(response)

    } catch (error) {
      return Result.fail<GetProblemSetListResponse>(`Unexpected error retrieving problem set list: ${error}`)
    }
  }

  private validateRequest(request: GetProblemSetListRequest): Result<void> {
    const errors: string[] = []
  ...
  private determineScopeByRole(request: GetProblemSetListRequest): Result<{
    canViewAll: boolean
  ...
  private async fetchProblemSets(
    request: GetProblemSetListRequest,
    scope: { canViewAll: boolean; canViewShared: boolean; ownTeacherId?: string }
  ): Promise<Result<{ problemSets: any[]; totalCount: number }>> {
    try {
      let problemSets: any[] = []
  ...
  private applyFilters(problemSets: any[], filters?: any): any[] {
    if (!filters) return problemSets
  ...
  private calculatePagination(page: number, limit: number, totalCount: number) {
  ...
  private mapToDto(problemSet: any, requesterRole: string): ProblemSetDto {
    const dto: ProblemSetDto = {
