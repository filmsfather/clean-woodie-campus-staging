 ...
 */
export class DailyStatsAggregationExecutor implements IBatchJobExecutor {
  public readonly supportedType: BatchJobType = 'daily_stats_aggregation'
  ...
  public async execute(job: BatchJob): Promise<Result<BatchJobExecutionResult>> {
    ...
    try {
      ...
      if (error) {
        ...
        return Result.fail<BatchJobExecutionResult>(`집계 함수 실행 실패: ${error.message}`)
      }
      ...
      return Result.fail<BatchJobExecutionResult>(`일별 통계 집계 실패: ${error}`)
    }
  }
}