 ...
 */
export class WeeklyStatsAggregationExecutor implements IBatchJobExecutor {
  public readonly supportedType: BatchJobType = 'weekly_stats_aggregation'
  ...
  public async execute(job: BatchJob): Promise<Result<BatchJobExecutionResult>> {
    ...
    try {
      ...
      if (dailyRecordsCount === 0) {
        ...
        return Result.fail<BatchJobExecutionResult>('해당 주에 대한 일별 집계 데이터가 존재하지 않습니다')
      }
      ...
      if (error) {
        return Result.fail<BatchJobExecutionResult>(`주별 집계 함수 실행 실패: ${error.message}`)
      }
      ...
      // 주간 통계 요약 계산
      const totalStudyDays = weeklyStats.reduce((sum: number, stat: any) => sum + stat.total_study_days, 0)
      const avgConsistencyScore = weeklyStats.length > 0 
        ? weeklyStats.reduce((sum: number, stat: any) => sum + (stat.consistency_score || 0), 0) / weeklyStats.length 
        : 0
      ...
      return Result.fail<BatchJobExecutionResult>(`주별 통계 집계 실패: ${error}`)
    }
  }
