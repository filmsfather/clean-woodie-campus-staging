...
import { Problem, ProblemContent, AnswerContent, Difficulty, Tag } from '@woodie/domain';
    ...
    async execute(request) {
        try {
            ...
            if (!request.teacherId?.trim()) {
                return ProblemUseCaseErrorFactory.requiredFieldMissing('teacherId');
            }
            if (!request.title?.trim()) {
                return ProblemUseCaseErrorFactory.requiredFieldMissing('title');
            }
            if (!request.correctAnswerValue?.trim()) {
                return ProblemUseCaseErrorFactory.requiredFieldMissing('correctAnswerValue');
            }
            // 2. Value Objects 생성
            ...
            if (contentResult.isFailure) {
                return Result.fail(`Invalid problem content: ${contentResult.error}`);
            }
            ...
            if (answerResult.isFailure) {
                return Result.fail(`Invalid answer content: ${answerResult.error}`);
            }
            ...
            if (request.tags && request.tags.length > 0) {
                for (const tagName of request.tags) {
                    ...
                    if (tagResult.isFailure) {
                        return Result.fail(`Invalid tag '${tagName}': ${tagResult.error}`);
                    }
            ...
            if (problemResult.isFailure) {
                return Result.fail(`Failed to create problem: ${problemResult.error}`);
            }
            const problem = problemResult.value;
            // 5. 저장 (도메인 오류 사용)
            const saveResult = await this.problemRepository.save(problem);
            if (saveResult.isFailure) {
                return ProblemUseCaseErrorFactory.repositoryError(`Failed to save problem: ${saveResult.error}`);
            }
            ...
            return ProblemUseCaseErrorFactory.unexpectedError(error instanceof Error ? error.message : 'Unknown error');
        }
    }
}
//# sourceMappingURL=CreateProblemUseCase.js.map