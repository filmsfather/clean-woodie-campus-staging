...
import { CacheKeyBuilder, CacheStrategies, CacheTags } from '../../common/interfaces/ICacheService';
...
export class ProblemAnalyticsService {
    ...
    async getProblemBankSummary(teacherId) {
        ...
        try {
            ...
            if (this.cacheService && stats.totalProblems > 0) {
                await this.cacheService.set(cacheKey, summaryDto, {
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.STATISTICS_CALCULATION_FAILED, 'Failed to calculate problem bank summary', { teacherId, correlationId }, error);
    ...
    async getTagAnalytics(teacherId) {
        const startTime = Date.now();
        const correlationId = this.generateCorrelationId();
        try {
            ...
            if (this.cacheService && uniqueTags.length > 0) {
                await this.cacheService.set(cacheKey, analyticsDto, {
                    ...CacheStrategies.getStatisticsOptions(),
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.TAG_ANALYSIS_FAILED, 'Failed to analyze tags', { teacherId, correlationId }, error);
    ...
    async getDifficultyAnalysis(teacherId) {
        const startTime = Date.now();
        const correlationId = this.generateCorrelationId();
        try {
            ...
            if (distributionResult.isFailure) {
                const error = new ProblemBankError(ProblemBankErrorCode.DIFFICULTY_ANALYSIS_FAILED, 'Failed to get difficulty distribution', { teacherId, correlationId }, new Error(distributionResult.error));
                return Result.fail(error.message);
            }
            ...
            // 결과 캐싱
            if (this.cacheService) {
                await this.cacheService.set(cacheKey, completeDistribution, {
                    ...CacheStrategies.getStatisticsOptions(),
                    tags: [CacheTags.forTeacher(teacherId), CacheTags.ANALYTICS]
                });
            }
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.DIFFICULTY_ANALYSIS_FAILED, 'Failed to analyze difficulty distribution', { teacherId, correlationId }, error);
    ...
    async getTypeDistribution(teacherId) {
        const startTime = Date.now();
        const correlationId = this.generateCorrelationId();
        try {
            ...
            const problemBankError = new ProblemBankError(ProblemBankErrorCode.STATISTICS_CALCULATION_FAILED, 'Failed to get type distribution', { teacherId, correlationId }, error);
    ...
    ensureCompleteDifficultyRange(distribution) {
        const result = [];
...
//# sourceMappingURL=ProblemAnalyticsService.js.map