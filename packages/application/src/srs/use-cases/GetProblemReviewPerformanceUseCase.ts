...
export interface GetProblemReviewPerformanceRequest {
  ...
  timeRangeInDays?: number // 기본값: 90일
}
...
export class GetProblemReviewPerformanceUseCase {
  ...
  async execute(request: GetProblemReviewPerformanceRequest): Promise<Result<GetProblemReviewPerformanceResponse>> {
    try {
      // 1. 입력 유효성 검증
      const validationResult = this.validateRequest(request)
      if (validationResult.isFailure) {
        return Result.fail<GetProblemReviewPerformanceResponse>(validationResult.error)
      }
      ...
      if (schedules.length === 0) {
        return Result.fail<GetProblemReviewPerformanceResponse>('No review schedules found for this problem')
      }
      ...
      // 9. 응답 구성
      const response: GetProblemReviewPerformanceResponse = {
      ...
      return Result.ok<GetProblemReviewPerformanceResponse>(response)

    } catch (error) {
      return Result.fail<GetProblemReviewPerformanceResponse>(`Failed to get problem review performance: ${error}`)
    }
  }
   ...
   * 입력 요청 유효성 검증
   */
  private validateRequest(request: GetProblemReviewPerformanceRequest): Result<void> {
   ...
   * 기본 성과 지표 계산
   */
  private calculateBasicMetrics(records: any[], schedules: any[]): {
    totalReviews: number
    averagePerformance: number
    difficultyTrend: 'improving' | 'stable' | 'declining'
    averageInterval: number
   ...
   * 난이도 분석
   */
  private analyzeDifficulty(records: any[], schedules: any[]): {
    currentLevel: 'easy' | 'medium' | 'hard' | 'very_hard'
    recommendedLevel: 'easy' | 'medium' | 'hard' | 'very_hard'
    reasonForRecommendation: string
    adjustmentSuggestion?: string
    ...
    // 현재 난이도 수준 판단
    let currentLevel: 'easy' | 'medium' | 'hard' | 'very_hard'
    ...
    // 권장 난이도 수준과 이유 결정
    let recommendedLevel: 'easy' | 'medium' | 'hard' | 'very_hard'
  ...
  private async analyzeStudentPerformance(
    schedules: any[],
    records: any[]
  ): Promise<StudentPerformanceBreakdown[]> {
    ...
    // 각 학생의 성과 분석
    const breakdown: StudentPerformanceBreakdown[] = []
  ...
  private calculateDifficultyTrend(records: any[]): 'improving' | 'stable' | 'declining' {
    if (records.length < 20) return 'stable'
  ...
  private calculatePerformanceVariance(studentBreakdown: StudentPerformanceBreakdown[]): number {
    if (studentBreakdown.length === 0) return 0
