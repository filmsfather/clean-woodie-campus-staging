...
export class GetRetentionProbabilityUseCase {
    ...
    async execute(request) {
        try {
            ...
            return Result.fail(`Failed to get retention probability: ${error}`);
        }
    }
    /**
     * 입력 요청 유효성 검증
    ...
    validateRequest(request) {
        ...
        if (request.scheduleIds && request.scheduleIds.some(id => !id || id.trim() === '')) {
            return Result.fail('All schedule IDs must be valid');
        }
        return Result.ok();
    }
    /**
     * 위험도 계산
     */
    calculateRiskLevel(probability, daysSinceLastReview, intervalDays) {
...
//# sourceMappingURL=GetRetentionProbabilityUseCase.js.map