...
export class GetStudentReviewStatsUseCase {
  ...
  async execute(request: GetStudentReviewStatsRequest): Promise<Result<GetStudentReviewStatsResponse>> {
    try {
      // 1. 입력 유효성 검증
      if (!request.studentId || request.studentId.trim() === '') {
        return Result.fail<GetStudentReviewStatsResponse>('Student ID is required')
      }
      ...
      // 6. 응답 구성
      const response: GetStudentReviewStatsResponse = {
      ...
      return Result.ok<GetStudentReviewStatsResponse>(response)

    } catch (error) {
      return Result.fail<GetStudentReviewStatsResponse>(`Failed to get student review stats: ${error}`)
    }
  }
   ...
   * 기본 통계 조회
   */
  private async getBasicStats(studentId: UniqueEntityID): Promise<StudentReviewStats> {
    // 모든 복습 스케줄 조회
    ...
    // 오늘 완료된 복습 조회
    const todayRecords = await this.studyRecordRepository.findByDateRange(
      studentId, todayStart, todayEnd
    )
   ...
   * 성과 지표 조회
   */
  private async getPerformanceMetrics(studentId: UniqueEntityID): Promise<{
    totalReviewsSinceStart: number
    averageAccuracy: number
    averageResponseTime?: number
    strongestDifficultyLevel: 'beginner' | 'intermediate' | 'advanced'
    ...
    const mostCommonPattern = Array.from(patternStats.entries())
      .sort(([,a], [,b]) => b - a)[0]?.[0] as any || 'quick_correct'
   ...
   * 트렌드 분석
   */
  private async getTrends(studentId: UniqueEntityID): Promise<{
    weeklyProgress: {
      week: string
      completedReviews: number
      accuracy: number
    ...
    const recentRecords = await this.studyRecordRepository.findByDateRange(
      studentId, fourWeeksAgo, now
    )
    ...
    // 연속 학습 트렌드
    const streakTrend = 'stable' as const // 실제 구현에서는 연속 학습 기록 분석
  ...
  private async calculateLongestStreak(studentId: UniqueEntityID): Promise<number> {
    // 간단한 구현 - 실제로는 연속 학습 기록을 추적해야 함
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
    const records = await this.studyRecordRepository.findByDateRange(
      studentId, thirtyDaysAgo, new Date()
    )
  ...
  private analyzeDifficultyPerformance(schedules: any[], records: any[]): {
    strongest: 'beginner' | 'intermediate' | 'advanced'
    weakest: 'beginner' | 'intermediate' | 'advanced'
  } {
    ...
    // 성과율 계산
    const performanceRates = new Map<string, number>()
    ...
    return {
      strongest: (sortedByPerformance[0]?.[0] as any) || 'beginner',
      weakest: (sortedByPerformance[sortedByPerformance.length - 1]?.[0] as any) || 'beginner'
    }
  }

  private calculateWeeklyProgress(records: any[]): {
    week: string
    completedReviews: number
    accuracy: number
    averageEaseFactor: number
  ...
  private async calculateMonthlyProgress(studentId: UniqueEntityID): Promise<{
    month: string
    completedReviews: number
    accuracy: number
    newCards: number
  ...
  private calculateImprovementTrend(records: any[]): 'improving' | 'stable' | 'declining' {
    ...
    else if (improvement < -0.05) return 'declining'
    else return 'stable'
  }
}