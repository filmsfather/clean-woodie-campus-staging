...
export interface ProcessNotificationQueueRequest {
  ...
  maxProcessingTimeMs?: number // 기본값: 30000 (30초)
}
...
export class ProcessNotificationQueueUseCase {
  ...
  async execute(request: ProcessNotificationQueueRequest): Promise<Result<ProcessNotificationQueueResponse>> {
    ...
    try {
      ...
      if (pendingNotificationsResult.isFailure) {
        return Result.fail<ProcessNotificationQueueResponse>(pendingNotificationsResult.error)
      }
      ...
      const response: ProcessNotificationQueueResponse = {
      ...
      return Result.ok<ProcessNotificationQueueResponse>(response)

    } catch (error) {
      return Result.fail<ProcessNotificationQueueResponse>(`Failed to process notification queue: ${error}`)
    }
  }
   ...
   * 개별 알림 처리
   */
  private async processSingleNotification(notification: any): Promise<{
    status: 'sent' | 'failed' | 'skipped'
    sentAt?: Date
    failureReason?: string
  }> {
   ...
   * 조용한 시간 확인
   */
  private isQuietHours(settings: any, currentTime: Date): boolean {
   ...
   * 알림 타입 활성화 여부 확인
   */
  private isNotificationTypeEnabled(settings: any, notificationType: string): boolean {
   ...
   * 다음 가능한 발송 시간 계산
   */
  private calculateNextAvailableTime(settings: any, currentTime: Date): Date {
    ...
    if (!quietStart || !quietEnd) return tomorrow
   ...
   * 큐 상태 조회
   */
  private async getQueueStatus(batchSize: number): Promise<{
    remainingInQueue: number
    nextBatchAvailable: boolean
    estimatedProcessingTime: number
  }> {
  ...
  private async rescheduleNotification(notification: any, newTime: Date): Promise<void> {
    notification.scheduledAt = newTime
    await this.notificationRepository.save(notification)
  }

  private async markNotificationAsCancelled(notification: any): Promise<void> {
    notification.status = 'cancelled'
    notification.cancelledAt = this.clock.now()
    await this.notificationRepository.save(notification)
  }

  private async markNotificationAsSent(notification: any, sentAt: Date): Promise<void> {
    notification.status = 'sent'
    notification.sentAt = sentAt
    await this.notificationRepository.save(notification)
  }

  private async handleSendFailure(notification: any, error: string): Promise<void> {
...
// 알림 발송 서비스 인터페이스 (Domain 레이어에 정의되어야 함)
