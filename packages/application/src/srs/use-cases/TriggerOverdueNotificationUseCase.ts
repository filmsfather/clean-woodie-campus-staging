...
export class TriggerOverdueNotificationUseCase {
  ...
  async execute(request: TriggerOverdueNotificationRequest): Promise<Result<TriggerOverdueNotificationResponse>> {
    try {
      // 1. 입력 유효성 검증
      const validationResult = this.validateRequest(request)
      if (validationResult.isFailure) {
        return Result.fail<TriggerOverdueNotificationResponse>(validationResult.error)
      }
      ...
      if (request.scheduleId) {
        ...
        schedules = await this.reviewScheduleRepository.findOverdueByStudentId(
          studentId, new Date()
        )
      } else {
        // 전체 연체 항목들 배치 처리
      ...
      const results: OverdueNotificationResult[] = []
      ...
      for (const schedule of schedules) {
        ...
        const result: OverdueNotificationResult = {
      ...
      // 4. 응답 구성
      const response: TriggerOverdueNotificationResponse = {
      ...
      return Result.ok<TriggerOverdueNotificationResponse>(response)

    } catch (error) {
      return Result.fail<TriggerOverdueNotificationResponse>(`Failed to trigger overdue notifications: ${error}`)
    }
  }
   ...
   * 입력 요청 유효성 검증
   */
  private validateRequest(request: TriggerOverdueNotificationRequest): Result<void> {
    if (request.batchSize !== undefined && request.batchSize <= 0) {
      return Result.fail<void>('Batch size must be a positive number')
    }

    if (request.batchSize !== undefined && request.batchSize > 1000) {
      return Result.fail<void>('Batch size cannot exceed 1000')
    }
