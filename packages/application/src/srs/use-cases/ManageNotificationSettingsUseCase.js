...
export class ManageNotificationSettingsUseCase {
    ...
    async execute(request) {
        try {
            ...
            if (notificationSettingsResult.isFailure) {
                return Result.fail(notificationSettingsResult.error);
            }
            ...
            // 6. 최적화 제안 생성
            const optimizationSuggestions = await this.generateOptimizationSuggestions(studentId, request.settings);
            // 7. 응답 구성
            const response = {
            ...
            return Result.fail(`Failed to manage notification settings: ${error}`);
        }
    }
    /**
     * 입력 요청 유효성 검증
    ...
    validateRequest(request) {
        ...
        if (settings.reminderMinutesBefore < 5 || settings.reminderMinutesBefore > 1440) {
            return Result.fail('Reminder minutes must be between 5 and 1440 (24 hours)');
        }
        // 연체 임계값 유효성
        if (settings.overdueThresholdHours < 1 || settings.overdueThresholdHours > 168) {
            return Result.fail('Overdue threshold must be between 1 and 168 hours (7 days)');
        }
        // 조용한 시간 형식 검증
        if (settings.quietHoursStart && !this.isValidTimeFormat(settings.quietHoursStart)) {
            return Result.fail('Invalid quiet hours start format. Use HH:MM format');
        }
        if (settings.quietHoursEnd && !this.isValidTimeFormat(settings.quietHoursEnd)) {
            return Result.fail('Invalid quiet hours end format. Use HH:MM format');
        }
        // 전달 방법 유효성
        if (settings.preferredDeliveryMethods.length === 0) {
            return Result.fail('At least one delivery method must be selected');
        }
        ...
        if (invalidMethods.length > 0) {
            return Result.fail(`Invalid delivery methods: ${invalidMethods.join(', ')}`);
        }
        return Result.ok();
    }
    /**
     ...
     * 유효성 경고 생성
     */
    generateValidationWarnings(settings) {
        const warnings = [];
        // 너무 빈번한 리마인더 경고
     ...
     * 최적화 제안 생성
     */
    async generateOptimizationSuggestions(studentId, settings) {
        const suggestions = [];
        try {
     ...
     * 학생 활동 패턴 분석 (간단한 버전)
     */
    async analyzeStudentActivityPattern(studentId) {
...
//# sourceMappingURL=ManageNotificationSettingsUseCase.js.map