...
export class AssessDifficultyLevelUseCase {
  ...
  async execute(request: AssessDifficultyLevelRequest): Promise<Result<AssessDifficultyLevelResponse>> {
    try {
      // 1. 입력 유효성 검증
      const validationResult = this.validateRequest(request)
      if (validationResult.isFailure) {
        return Result.fail<AssessDifficultyLevelResponse>(validationResult.error)
      }
      ...
      if (schedules.length === 0) {
        return Result.fail<AssessDifficultyLevelResponse>('No review schedules found')
      }

      // 3. 각 스케줄에 대한 난이도 평가
      const items: DifficultyAssessmentItem[] = []

      for (const schedule of schedules) {
        // 해당 문제의 최근 학습 기록 조회
        const records = await this.studyRecordRepository.findByStudentAndProblem(
          studentId,
          schedule.problemId
        )

        let averagePerformanceScore: number | undefined
        let lastReviewPattern: string | undefined

        if (records.length > 0) {
          averagePerformanceScore = records.reduce((sum: number, record: any) => 
            sum + record.calculatePerformanceScore(), 0) / records.length
          
          const lastRecord = records[0]
          lastReviewPattern = (lastRecord as any).getStudyPattern().pattern
        }

        const item: DifficultyAssessmentItem = {
      ...
      // 6. 응답 구성
      const response: AssessDifficultyLevelResponse = {
      ...
      return Result.ok<AssessDifficultyLevelResponse>(response)

    } catch (error) {
      return Result.fail<AssessDifficultyLevelResponse>(`Failed to assess difficulty level: ${error}`)
    }
  }
   ...
   * 입력 요청 유효성 검증
   */
  private validateRequest(request: AssessDifficultyLevelRequest): Result<void> {
  ...
  private determineSuggestedAction(
    ...
    averagePerformanceScore?: number,
    lastReviewPattern?: string
  ): 'continue' | 'increase_frequency' | 'review_fundamentals' | 'consider_advanced' {
   ...
   * 전체 요약 계산
   */
  private calculateOverallSummary(items: DifficultyAssessmentItem[]): {
    beginnerCount: number
    intermediateCount: number
    advancedCount: number
    averageEaseFactor: number
    ...
    const strugglingItems = items.filter(item => 
      item.consecutiveFailures >= 2 || 
      (item.averagePerformanceScore !== undefined && item.averagePerformanceScore < 60)
    ).length
