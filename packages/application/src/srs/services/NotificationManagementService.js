...
export class NotificationManagementService {
    ...
    historyRepository;
    clock;
    constructor(notificationService, // Supabase Realtime 구현체
    settingsRepository, // 설정 저장소
    historyRepository, // 이력 저장소
    clock // 시간 서비스
    ) {
     ...
     * 로그인 시 또는 앱 시작 시 호출
     */
    async initializeUserNotifications(userId) {
        try {
            ...
            if (!settings.enabled) {
                return Result.fail('Notifications are disabled for this user');
            }
            // 3. 실시간 채널 구독
            ...
            return Result.ok(subscriptionResult.getValue());
        }
        catch (error) {
            return Result.fail(`Failed to initialize user notifications: ${error}`);
        }
    }
    /**
     * 즉시 알림 전송
     ...
     */
    async sendImmediateNotification(recipientId, type, title, body, data) {
        try {
            // 1. 사용자 알림 설정 확인
            const settings = await this.settingsRepository.findByUserId(recipientId);
            if (!settings || !settings.enabled) {
                return Result.fail('User has notifications disabled');
            }
            // 2. 알림 타입별 설정 확인
            const category = type.getCategory();
            if (!settings.isNotificationTypeEnabled(category)) {
                return Result.fail(`User has disabled ${category} notifications`);
            }
            // 3. 조용한 시간 확인 (긴급 알림 제외)
            if (!type.isUrgent() && settings.isQuietTime(this.clock.now())) {
                return Result.fail('Current time is within user quiet hours');
            }
            // 4. 알림 메시지 생성
            const message = {
            ...
            return Result.fail(`Failed to send immediate notification: ${error}`);
        }
    }
    /**
     * 예약 알림 전송 (배치)
     * 스케줄러나 백그라운드 작업에서 호출
     */
    async sendScheduledNotifications(requests) {
        try {
            let successCount = 0;
            const messages = [];
            // 1. 각 요청을 알림 메시지로 변환
            for (const request of requests) {
                ...
                if (!settings.isNotificationTypeEnabled(category))
                    continue;
                // 조용한 시간 확인 (긴급 알림 제외)
                if (!request.type.isUrgent() && settings.isQuietTime(request.scheduledFor))
                    continue;
                // 알림 메시지 생성
                const message = {
                    id: this.generateNotificationId(),
            ...
            return Result.fail(`Failed to send scheduled notifications: ${error}`);
        }
    }
    /**
     * 사용자 알림 설정 업데이트
    ...
    async updateNotificationSettings(userId, updates) {
        try {
            ...
            return Result.fail(`Failed to update notification settings: ${error}`);
        }
    }
    /**
     * 사용자 알림 통계 조회
    ...
    async getNotificationStatistics(userId) {
        try {
            ...
            return Result.fail(`Failed to get notification statistics: ${error}`);
        }
    }
    /**
     * 알림 채널 정리 (로그아웃 시)
    ...
    async cleanupUserNotifications(userId) {
        try {
            ...
            return Result.fail(`Failed to cleanup user notifications: ${error}`);
        }
    }
    /**
     * 고유한 알림 ID 생성
...
//# sourceMappingURL=NotificationManagementService.js.map