...
export class UseInviteTokenUseCase implements UseCase<UseInviteTokenDto, InviteDto> {
  constructor(
    private inviteRepository: IInviteRepository
  ) {}

  async execute(dto: UseInviteTokenDto): Promise<Result<InviteDto>> {
    try {
      // 토큰 값 객체 생성 및 검증
      const tokenResult = InviteToken.createFromValue(dto.token);
      if (tokenResult.isFailure) {
        return Result.fail<InviteDto>(`Invalid token format: ${tokenResult.errorValue}`);
      }

      // 리포지토리에서 토큰으로 초대 조회
      const inviteResult = await this.inviteRepository.findByToken(tokenResult.value);
      if (inviteResult.isFailure) {
        return Result.fail<InviteDto>(`Failed to find invite: ${inviteResult.errorValue}`);
      }
      ...
      // 초대가 존재하지 않는 경우
      if (!invite) {
        return Result.fail<InviteDto>('Invalid token');
      }

      // 도메인 로직을 통해 토큰 사용 처리
      const markUsedResult = invite.markAsUsed(dto.userId);
      if (markUsedResult.isFailure) {
        return Result.fail<InviteDto>(`Failed to use token: ${markUsedResult.errorValue}`);
      }

      // 변경사항을 리포지토리에 저장
      const savedInviteResult = await this.inviteRepository.save(invite);
      if (savedInviteResult.isFailure) {
        return Result.fail<InviteDto>(`Failed to save used invite: ${savedInviteResult.errorValue}`);
      }
      ...
      return Result.fail<InviteDto>(`Unexpected error using invite token: ${error}`);
    }
  }
}