...
export class CacheWarmerService {
    ...
    problemService;
    config;
    constructor(cacheService, progressService, problemService, config) {
     ...
     * 인기 문제들 캐시 워밍
     */
    async warmPopularProblems(result) {
        try {
     ...
     * 최근 집계 데이터 캐시 워밍
     */
    async warmRecentAggregates(result) {
        try {
     ...
     * 특정 학생의 캐시 프리로딩
     */
    async warmStudentCache(studentId) {
        try {
            // 병렬로 학생 관련 데이터 로드
     ...
     * 특정 교사의 캐시 프리로딩
     */
    async warmTeacherCache(teacherId) {
        try {
            // 교사 관련 데이터 로드
            await Promise.allSettled([
...
//# sourceMappingURL=CacheWarmerService.js.map