...
export class ReviewQueueService {
    ...
    clock;
    constructor(reviewScheduleRepository, // Domain 인터페이스
    studyRecordRepository, // Domain 인터페이스  
    spacedRepetitionPolicy, // Domain 인터페이스
    clock // Domain 인터페이스
    ) {
        this.reviewScheduleRepository = reviewScheduleRepository;
        this.studyRecordRepository = studyRecordRepository;
        this.spacedRepetitionPolicy = spacedRepetitionPolicy;
        this.clock = clock;
    }
    /**
    ...
    async getTodayReviews(studentId) {
        try {
            ...
            // 오늘 복습해야 할 항목들 조회
            const schedules = await this.reviewScheduleRepository.findTodayReviews(studentId, currentDate);
            // DTO로 변환하고 우선순위 계산
            const queueItems = schedules.map(schedule => this.toQueueItem(schedule));
            // 우선순위별 정렬
            const sortedItems = this.sortByPriority(queueItems);
            return Result.ok(sortedItems);
        }
        catch (error) {
    ...
    async getOverdueReviews(studentId) {
        try {
            const currentDate = this.clock.now();
            const schedules = await this.reviewScheduleRepository.findOverdueReviews(studentId, currentDate);
            const queueItems = schedules
                .map(schedule => this.toQueueItem(schedule))
                .filter(item => item.isOverdue);
            // 연체일이 긴 순서대로 정렬
            const sortedItems = queueItems.sort((a, b) => a.nextReviewAt.getTime() - b.nextReviewAt.getTime());
            return Result.ok(sortedItems);
        }
        catch (error) {
     ...
     */
    async markReviewCompleted(studentId, scheduleId, feedback, responseTime, answerContent) {
        try {
            // 1. 복습 일정 조회
            ...
            if (!schedule.studentId.equals(studentId)) {
                return Result.fail('Unauthorized access to review schedule');
            }
            // 2. 이전 상태 백업 (결과 DTO용)
            const previousInterval = schedule.currentInterval;
            const previousEaseFactor = schedule.easeFactor;
            // 3. 도메인 로직 실행 (이벤트 발행됨)
            const processingResult = schedule.processReviewFeedback(feedback, this.spacedRepetitionPolicy, this.clock, { responseTime, answerContent } // StudyRecord 생성 정보
            );
            ...
            return Result.fail(`Failed to complete review: ${error}`);
        }
    }
    /**
     * 복습 통계 조회
    ...
    async getReviewStatistics(studentId) {
        try {
            ...
            // 병렬로 통계 데이터 조회
            const [totalScheduled, dueToday, overdue, todayRecords, recentRecords] = await Promise.all([
     ...
     * 연속 학습 일수 계산
     */
    calculateStreakDays(records, currentDate) {
        if (records.length === 0)
            return 0;
        // 날짜별로 그룹화
     ...
     * 평균 정답률 계산
     */
    calculateAverageRetention(records) {
        if (records.length === 0)
            return 0;
     ...
     * 총 학습 시간 계산 (분 단위)
     */
    calculateTotalTimeSpent(records) {
        return records
