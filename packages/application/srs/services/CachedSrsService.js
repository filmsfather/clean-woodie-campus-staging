 ...
 */
export class CachedSrsService {
    reviewScheduleRepository;
    studyRecordRepository;
    cacheService;
    srsPolicy;
    clock;
    constructor(reviewScheduleRepository, studyRecordRepository, cacheService, srsPolicy, clock) {
        this.reviewScheduleRepository = reviewScheduleRepository;
        this.studyRecordRepository = studyRecordRepository;
        this.cacheService = cacheService;
        this.srsPolicy = srsPolicy;
        this.clock = clock;
    }
    /**
    ...
    async getTodayReviews(studentId) {
        ...
        try {
            ...
            if (cachedReviews) {
                return Result.ok(cachedReviews.map(data => ReviewSchedule.reconstitute(data, new UniqueEntityID(data.id.toString()))));
            }
            // 2. 캐시 미스 시 DB에서 조회
     ...
     * 복습 완료 처리 (캐시 무효화 적용)
     */
    async completeReview(reviewScheduleId, feedback, studyInfo) {
        try {
            // 1. 복습 일정 조회
            ...
            // 2. 복습 피드백 처리
            const processResult = reviewSchedule.processReviewFeedback(feedback, this.srsPolicy, this.clock, studyInfo);
     ...
     * 학생의 복습 통계 조회 (캐싱 적용)
     */
    async getStudentReviewStats(studentId) {
        ...
        try {
            ...
            // 평균 ease factor 계산
            const allSchedules = await this.reviewScheduleRepository.findDueReviews(studentId, new Date(Date.now() + 365 * 24 * 60 * 60 * 1000) // 1년 후까지 모든 카드
            );
     ...
     * 문제별 복습 성과 조회 (캐싱 적용)
     */
    async getProblemReviewPerformance(problemId) {
        const cacheKey = CacheKeys.SRS_PROBLEM_PERFORMANCE(problemId.toString());
        try {
            // 1. 캐시에서 먼저 조회
            const cachedPerformance = await this.cacheService.get(cacheKey);
            ...
            // 시간 순으로 정렬하여 트렌드 계산
            const sortedRecords = studyRecords.sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
     ...
     * 새로운 복습 일정 생성 (캐시 무효화)
     */
    async createReviewSchedule(studentId, problemId) {
        try {
            ...
            // 새로 생성된 일정 반환
            return Result.ok({}); // 실제 구현에서는 생성된 ReviewSchedule 반환
        }
        catch (error) {
     ...
     * 학생의 SRS 관련 캐시 무효화
     */
    async invalidateStudentSrsCache(studentId) {
     ...
     * 오늘의 학습 기록 조회 헬퍼
     */
    async getTodayStudyRecords(studentId) {
        try {
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const tomorrow = new Date(today);
            tomorrow.setDate(tomorrow.getDate() + 1);
            const records = await this.studyRecordRepository.findByDateRange(studentId, today, tomorrow);
...
//# sourceMappingURL=CachedSrsService.js.map