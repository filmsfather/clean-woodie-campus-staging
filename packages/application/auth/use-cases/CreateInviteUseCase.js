import { Result, Email, Invite } from '@woodie/domain';
export class CreateInviteUseCase {
    inviteRepository;
    notificationService;
    constructor(inviteRepository, notificationService) {
    ...
    async execute(dto) {
        try {
            ...
            const duplicateCheckResult = await this.inviteRepository.hasActivePendingInvite(emailResult.value, dto.organizationId);
            if (duplicateCheckResult.isFailure) {
                return Result.fail(`Failed to check duplicate invite: ${duplicateCheckResult.errorValue}`);
            }
            if (duplicateCheckResult.value) {
                return Result.fail('An active invite already exists for this email in this organization');
            }
            // 초대 엔티티 생성
            ...
            if (inviteResult.isFailure) {
                return Result.fail(`Failed to create invite: ${inviteResult.errorValue}`);
            }
            // 리포지토리에 저장
            const savedInviteResult = await this.inviteRepository.save(inviteResult.value);
            if (savedInviteResult.isFailure) {
                return Result.fail(`Failed to save invite: ${savedInviteResult.errorValue}`);
            }
            // 초대 이메일 발송 (사이드 이펙트)
            ...
            return Result.fail(`Unexpected error creating invite: ${error}`);
        }
    }
}
//# sourceMappingURL=CreateInviteUseCase.js.map