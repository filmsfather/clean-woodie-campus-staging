 ...
 */
export class ProblemSetStatsExecutor {
    databaseClient;
    supportedType = 'problem_set_stats';
    ...
    async execute(job) {
        ...
        try {
            ...
            if (specificProblemSetIds && Array.isArray(specificProblemSetIds)) {
                ...
                if (error) {
                    return Result.fail(`문제집 통계 집계 함수 실행 실패: ${error.message}`);
                }
        ...
        `);
            ...
            return Result.fail(`문제집 통계 집계 실패: ${error}`);
        }
    }
    /**
     * 개별 문제집 통계 업데이트 (특정 문제집 처리용)
     */
    async updateProblemSetStats(problemSetId) {
        // 특정 문제집에 대한 통계 재계산
     ...
     * 문제집별 통계 계산
     */
    async calculateProblemSetStats(problemSetId) {
...
//# sourceMappingURL=ProblemSetStatsExecutor.js.map