...
import { PerformanceConfigLoader } from '../../config/PerformanceConfig';
...
export class PerformanceOptimizationService {
    ...
    thresholds;
    rateLimit;
    constructor(cacheService, performanceMonitor, assetManager) {
        ...
        this.rateLimiter = new TokenBucketRateLimiter(this.rateLimit.optimization.maxConcurrentJobs, 1 / (this.rateLimit.optimization.cooldownMinutes * 60) // 쿨다운 기간을 고려한 rate
        );
    }
    /**
     * 성능 분석 및 최적화 추천사항 생성
    ...
    async executeOptimizations(recommendations) {
        ...
        if (!rateLimitResult.allowed) {
            return Result.fail(`최적화 실행이 일시적으로 제한되었습니다. ${rateLimitResult.retryAfter}초 후 다시 시도해주세요.`);
        }
